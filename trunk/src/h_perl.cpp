...
int Hilit_PERL(EBuffer *BF, int /*LN*/, PCell B, int Pos, int Width,
...
#define MAXSEOF 100
    ...
    for(i = 0; i < Line->Count;) {
        ...
        IF_TAB() else {
		    ...
		    if (p[1] != '}' && !(x < Line->Count+1 && strncmp(Line->Chars + x, "=>", 2) == 0))
		    {
			...
			if (j == 1) { 
				...
				op = -1;
                    ...
                    if (
                        (var_type == '$' && strchr("_&`'+*.!/|,\\\";#%=-~:?<>()[]", first)
                         != NULL) || (var_type == '@' && strchr("_", first) != NULL)
                       )
                    {
                        // nothing.
                    }
                    ...
                    Color = CLR_Number;
                    ColorNext();
                    while (len && (isdigit(*p) || (*p == 'e' || *p == 'E' || *p == '_')))
                        ColorNext();
                    State = hsPerl_Normal | X_BIT;
                    continue;
                } else if (*p == '\'') {
                    ...
                    if (p[offset] == '_' || (toupper(p[offset]) >= 'A' &&
                                             toupper(p[offset]) <= 'Z'))
                    {
                        int hereDocKeyLen;
                        setHereDoc++;
                        for (hereDocKeyLen = 0;
                             ...
                             (p[offset + hereDocKeyLen] == '_' ||
                              (toupper(p[offset + hereDocKeyLen]) >= 'A' &&
                               toupper(p[offset + hereDocKeyLen]) <= 'Z'));
                             ++hereDocKeyLen)
                        {
