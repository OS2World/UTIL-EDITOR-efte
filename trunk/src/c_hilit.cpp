...
{
    for (unsigned int i = 0; i < sizeof(IndentModes) / sizeof(IndentModes[0]);
	 i++)
...
{
    for (unsigned int i = 0; i < sizeof(HilitModes) / sizeof(HilitModes[0]);
	 i++)
...
int EBuffer::HilitAddWord(const char *Word)
{
	...
	return 1;
    WordList =
	(char **)realloc((void *)WordList, (1 + WordCount) * sizeof(char *));
    if (WordList == 0)
	return 0;
...
int EBuffer::HilitRemoveWord(const char *Word)
{
	...
	free(WordList[i]);
	memmove(WordList + i, WordList + i + 1,
		sizeof(char *) * (WordCount - i - 1));
	WordCount--;
	WordList =
	    (char **)realloc((void *)WordList, WordCount * sizeof(char *));
	FullRedraw();
	return 1;
    }
    return 0;
...
HMachine::~HMachine()
{
    ...
    if (trans) {
	while (transCount--) {
	    if (trans[transCount].match)
		free(trans[transCount].match);
	    if (trans[transCount].regexp)
		RxFree(trans[transCount].regexp);
	}
