...
int Hilit_MSG(EBuffer * BF, int /*LN*/, PCell B, int Pos, int Width,
	      ELine * Line, hlState & State, hsState * StateMap, int *ECol)
{
	...
	if (State == hsMSG_Normal) {
		...
		is_tag = 1;
		     ...
		     && memcmp(Line->Chars, " * Origin:", 10) == 0)
		is_control = 1;
	    else if (Line->Count > 0 && Line->Chars[0] == '\x01')
		is_control = 1;
	    else
		for (i = 0; i < Line->Count; i++) {
		    if (i < 30 && Line->Chars[i] == ':' && i < Line->Count - 1
			&& Line->Chars[i + 1] == ' ' && !is_space) {
			is_head = 1;
			break;
		    }
    ...
    if (BF->Mode->fColorize->Keywords.TotalCount > 0 || BF->WordCount > 0) {	/* words have to be hilited, go slow */
