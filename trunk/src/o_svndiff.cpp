...
ESvnDiff::ESvnDiff(int createFlags, EModel ** ARoot, char *ADir,
		   char *ACommand, char *AOnFiles):ESvnBase(createFlags,
							    ARoot, "SVN diff")
{
    SvnDiffView = this;
    CurrFile = 0;
...
void ESvnDiff::ParseFromTo(char *line, int /*len */ )
{
    char *start;
    char *end;
...
void ESvnDiff::ParseLine(char *line, int len)
{
...
int ESvnDiff::RunPipe(char *ADir, char *ACommand, char *AOnFiles)
{
    FreeLines();
    free(CurrFile);
    CurrLine = ToLine = InToFile = 0;
...
int ESvnDiff::ExecCommand(int Command, ExState & State)
{
    switch (Command) {
    case ExBlockCopy:
	return BlockCopy(0);
...
int ESvnDiff::BlockCopy(int Append)
{
    if (SSBuffer == 0)
	return ErFAIL;
    if (Append) {
