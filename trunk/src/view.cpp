...
int EView::FilePrev()
{
	...
	if (IgnoreBufferList && n && n->GetContext() == CONTEXT_BUFFERS)
	    n = n->Prev;
	SelectModel(n);
	return 1;
    }
...
int EView::SwitchTo(ExState & State)
{
	...
	if (MView->Win->GetStr("Obj.Number", sizeof(str), (char *)str, 0) ==
	    0)
	    return 0;
	No = atoi(str);
    }
...
int EView::FileOpen(ExState & State)
{
	...
	if (GetDefaultDirectory(Model, FName, sizeof(FName)) == 0)
	    return 0;
	if (MView->Win->
	    GetFile("Open file", sizeof(FName), FName, HIST_PATH,
		    GF_OPEN) == 0)
	    return 0;
    }
...
int EView::ViewRoutines(ExState & /*State */ )
{
	...
	if (BFS(Buffer, BFS_RoutineRegexp) == 0) {
	    MView->Win->Choice(GPC_ERROR, "Error", 1, "O&K",
			       "No routine regexp.");
	    return 0;
	}
...
int EView::TagLoad(ExState & State)
{
	...
	if (MView->Win->
	    GetFile("Load tags", sizeof(Tag), Tag, HIST_TAGFILES,
		    GF_OPEN) == 0)
	    return 0;
...
#ifdef UNIX
    if (ExpandPath
	("~/.efterc", command + strlen(command),
	 sizeof(command) - strlen(command)) != 0)
	return 0;
#else
    strlcat(command, "\"", sizeof(command));
...
int EView::RemoveGlobalBookmark(ExState & State)
{
	...
	if (MView->Win->
	    GetStr("Remove Global Bookmark", sizeof(name), name,
		   HIST_BOOKMARK) == 0)
	    return 0;
...
int EView::GotoGlobalBookmark(ExState & State)
{
    ...
    if (State.GetStrParam(this, name, sizeof(name)) == 0)
	if (MView->Win->
	    GetStr("Goto Global Bookmark", sizeof(name), name,
		   HIST_BOOKMARK) == 0)
	    return 0;
...
int EView::Cvs(ExState & State)
{
	...
	if (MView->Win->GetStr("CVS options", sizeof(Opts), Opts, HIST_CVS) ==
	    0)
	    return 0;
	strcpy(Options, Opts);
    }
	...
	if (MView->Win->
	    GetStr("CVS options", sizeof(Options), Options, HIST_CVS) == 0)
	    return 0;
    }
    return Cvs(Options);
}
...
int EView::Cvs(char *Options)
{
    ...
    case CONTEXT_FILE:
	if (JustFileName(((EBuffer *) Model)->FileName, OnFiles, sizeof(buf))
	    != 0)
	    return 0;			// OnFiles points to buf
	break;
	...
	if (!OnFiles)
	    OnFiles = strdup(((ECvs *) Model)->OnFiles);
	break;
    }
...
int EView::CvsDiff(ExState & State)
{
    ...
    if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
	if (MView->Win->
	    GetStr("CVS diff options", sizeof(Opts), Opts, HIST_CVSDIFF) == 0)
	    return 0;
	strcpy(Options, Opts);
    }
    else {
	if (MView->Win->
	    GetStr("CVS diff options", sizeof(Options), Options,
		   HIST_CVSDIFF) == 0)
	    return 0;
    }
...
int EView::CvsCommit(ExState & State)
{
    ...
    if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
	if (MView->Win->
	    GetStr("CVS commit options", sizeof(Opts), Opts,
		   HIST_CVSCOMMIT) == 0)
	    return 0;
	strcpy(Options, Opts);
    }
	...
	if (MView->Win->
	    GetStr("CVS commit options", sizeof(Options), Options,
		   HIST_CVSCOMMIT) == 0)
	    return 0;
    }
...
int EView::Svn(ExState & State)
{
    ...
    if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
	if (MView->Win->GetStr("SVN options", sizeof(Opts), Opts, HIST_SVN) ==
	    0)
	    return 0;
	strcpy(Options, Opts);
    }
	...
	if (MView->Win->
	    GetStr("SVN options", sizeof(Options), Options, HIST_SVN) == 0)
	    return 0;
    }
    return Svn(Options);
}
...
int EView::Svn(char *Options)
{
	...
	if (!OnFiles)
	    OnFiles = strdup(((ESvn *) Model)->OnFiles);
	break;
    }
...
int EView::SvnDiff(ExState & State)
{
    ...
    if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
	if (MView->Win->
	    GetStr("SVN diff options", sizeof(Opts), Opts, HIST_SVNDIFF) == 0)
	    return 0;
	strcpy(Options, Opts);
    }
    else {
	if (MView->Win->
	    GetStr("SVN diff options", sizeof(Options), Options,
		   HIST_SVNDIFF) == 0)
	    return 0;
    }
...
int EView::SvnCommit(ExState & State)
{
    ...
    if (State.GetStrParam(this, Options, sizeof(Options)) == 0) {
	if (MView->Win->
	    GetStr("SVN commit options", sizeof(Opts), Opts,
		   HIST_SVNCOMMIT) == 0)
	    return 0;
	strcpy(Options, Opts);
    }
	...
	if (MView->Win->
	    GetStr("SVN commit options", sizeof(Options), Options,
		   HIST_SVNCOMMIT) == 0)
	    return 0;
    }
