...
//    printf("Showing row %d\n", Row);
	...
	memmove(VV + VAllocated - (VCount - VGap),
		VV + VGap, sizeof(int) * (VCount - VGap));
    }
    if (VGap != V + 1)
	if (MoveVGap(V + 1) == 0)
	    return 0;
...
{					/*FOLD00 */
	...
	memmove(VV + VGap + GapSize - VAllocated / 3,
		VV + VGap + GapSize, sizeof(int) * (VCount - VGap));
...
int EBuffer::FoldCreateAtRoutines()
{					/*FOLD00 */
...
{					/*FOLD00 */
    ...
    if (BFI(this, BFI_Undo)) {
	if (PushULong(FF[f].level) == 0)
	    return 0;
	if (PushULong(Line) == 0)
	    return 0;
	if (PushUChar(ucFoldDestroy) == 0)
	    return 0;
    }
...
{					/*FOLD00 */
    ...
    while (f + 1 < FCount && FF[f + 1].level > level)
	f++;
...
{					/*FOLD00 */
    ...
    if (SetPosR(CP.Col, FF[f].line, tmLeft) == 0)
	return 0;

    if (BFI(this, BFI_Undo)) {
	if (PushULong(Line) == 0)
	    return 0;
	if (PushUChar(ucFoldClose) == 0)
	    return 0;
    }
    ...
    top = FF[f].line;
    level = FF[f].level;
    while ((f < FCount - 1) && (FF[f + 1].level > level))
	f++;
