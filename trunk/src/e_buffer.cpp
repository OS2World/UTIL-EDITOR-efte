...
PrevPos(-1, -1), SavedPos(-1, -1), Match(-1, -1)
{
    ...
    if (Mode == NULL) {
	fprintf(stderr,
		"Configuration error: default mode does not exist.\n");
	exit(1);
    }
...
int EBuffer::Modify()
{
	...
	if ((FileName != 0) && FileOk && (stat(FileName, &StatBuf) == 0)) {
		...
		//                         FileName);
		switch (View->MView->Win->
			Choice(GPC_WARNING, "Warning! File Changed on Disk",
...
int EBuffer::LoadRegion(EPoint * /*A*/, int /*FH*/, int /*StripChar */ ,
			int /*LineChar */ )
{
    return 0;
}
...
int EBuffer::UpdateMark(EPoint & M, int Type, int Row, int Col, int Rows,
			int Cols)
{
    switch (Type) {
...
{
    if ((Pos.Col >= 0) && (Pos.Row >= 0) && (Pos.Row < VCount))
	return 1;
    return 0;
}
...
{
    if ((Pos.Col >= 0) && (Pos.Row >= 0) && (Pos.Row < RCount))
	return 1;
    return 0;
}
...
int EBuffer::SetPos(int Col, int Row, int tabMode)
{
    ...
    PrevPos = CP;
    PrevPos.Row =
	(CP.Row < VCount) ? VToR(CP.Row) : (CP.Row - VCount + RCount);
    CP.Row = Row;
    ...
    if (ExtendGrab == 0 && AutoExtend == 0
	&& BFI(this, BFI_PersistentBlocks) == 0) {
	if (CheckBlock() == 1)
	    if (BlockUnmark() == 0)
		return 0;
    }
...
//    printf("DelLine: %d\n", Row);
	...
	if (PushUData(RLine(Row)->Chars, RLine(Row)->Count) == 0)
	    return 0;
	if (PushULong(RLine(Row)->Count) == 0)
	    return 0;
	if (PushULong(Row) == 0)
	    return 0;
	if (PushUChar(ucDelLine) == 0)
	    return 0;
    }
    if (DoMark)
	...
	memmove(LL + RGap + GapSize - RAllocated / 3,
		LL + RGap + GapSize, sizeof(PELine) * (RCount - RGap));
	if (Allocate(RAllocated - RAllocated / 3) == 0)
	    return 0;
    }
	...
	memmove(VV + VGap + GapSize - VAllocated / 3,
		VV + VGap + GapSize, sizeof(VV[0]) * (VCount - VGap));
	if (AllocVis(VAllocated - VAllocated / 3) == 0)
	    return 0;
    }
    return 1;
}
...
//    printf("++ %d G:C:A :: %d - %d - %d\n", Row, RGap, RCount, RAllocated);
	...
	memmove(LL + RAllocated - (RCount - RGap),
		LL + RGap, sizeof(PELine) * (RCount - RGap));
    }
    if (RGap != Row)
	if (MoveRGap(Row) == 0)
	    return 0;
	...
	if (AllocVis(VCount ? (VCount * 2) : 1) == 0)
	    return 0;
	memmove(VV + VAllocated - (VCount - VGap),
		VV + VGap, sizeof(VV[0]) * (VCount - VGap));
    }
    if (VGap != VLine)
	if (MoveVGap(VLine) == 0)
	    return 0;
...
//    printf("DelChars: %d:%d %d\n", Row, Ofs, ACount);
    ...
    if (BFI(this, BFI_Undo) == 1) {
	if (PushUData(L->Chars + Ofs, ACount) == 0)
	    return 0;
	if (PushULong(ACount) == 0)
	    return 0;
	if (PushULong(Ofs) == 0)
	    return 0;
	if (PushULong(Row) == 0)
	    return 0;
	if (PushUChar(ucDelChars) == 0)
	    return 0;
    }

    if (L->Count > Ofs + ACount)
	memmove(L->Chars + Ofs, L->Chars + Ofs + ACount,
		L->Count - Ofs - ACount);
    L->Count -= ACount;
    if (L->Allocate(L->Count) == 0)
	return 0;
    Draw(Row, Row);
    Hilit(Row);
//    printf("OK\n");
...
//    printf("InsChars: %d:%d %d\n", Row, Ofs, ACount);
	...
	if (PushULong(ACount) == 0)
	    return 0;
	if (PushUChar(ucInsChars) == 0)
	    return 0;
    }
    if (L->Allocate(L->Count + ACount) == 0)
	return 0;
    if (L->Count > Ofs)
...
//    printf("DelTExt: %d:%d %d\n", Row, Col, ACount);
    ...
    if ((ACount > 0) && (B != -1) && (C != -1)) {
	if (DelChars(Row, B, C - B) == 0)
	    return 0;
	if (DoMark)
	    UpdateMarker(umDelete, Row, Col, 0, ACount);
    }
//    printf("OK\n");
    return 1;
}

int EBuffer::InsText(int Row, int Col, int ACount, const char *ABuffer,
		     int DoMark)
{
    int B, L;
...
//    printf("InsText: %d:%d %d\n", Row, Col, ACount);
    ...
    if (DoMark)
	UpdateMarker(umInsert, Row, Col, 0, ACount);
    L = LineLen(Row);
    if (L < Col) {
	if (InsertIndent(Row, RLine(Row)->Count, Col - L) == 0)
	    return 0;
    }
    else if (UnTabPoint(Row, Col) == 0)
	return 0;
    B = CharOffset(RLine(Row), Col);
    if (InsChars(Row, B, ACount, ABuffer) == 0)
	return 0;
//    printf("OK\n");
    return 1;
}
...
int EBuffer::InsLineText(int Row, int Col, int ACount, int LCol, PELine Line)
{
    ...
    if (C <= 0)
	return 1;
    if (InsText(Row, Col, C, Line->Chars + B) == 0)
	return 0;
//    printf("OK\n");
    return 1;
}
...
int EBuffer::SplitLine(int Row, int Col)
{
	...
	VL = RToV(Row - 1);
	if (VL == -1)
	    if (ExposeRow(Row - 1) == 0)
		return 0;
    }
    VL = RToV(Row);
    assert(VL != -1);
    if (Col == 0) {
	if (InsLine(Row, 0, 1) == 0)
	    return 0;
    }
    else {
	UpdateMarker(umSplitLine, Row, Col, 0, 0);
	if (InsLine(Row, 1, 0) == 0)
	    return 0;
	...
	if (Col < LineLen(Row)) {
	    ...
	    if (InsText
		(Row + 1, 0, RLine(Row)->Count - P, RLine(Row)->Chars + P,
		 0) == 0)
		return 0;
	    if (DelText(Row, Col, L - Col, 0) == 0)
		return 0;
	}
    }
    Draw(Row, -1);
...
int EBuffer::JoinLine(int Row, int Col)
{
	...
	if (InsText(Row, Col, RLine(Row + 1)->Count, RLine(Row + 1)->Chars, 0)
	    == 0)
	    return 0;
	if (DelLine(Row + 1, 0) == 0)
	    return 0;
