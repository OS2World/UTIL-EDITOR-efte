...
void EventMapView::DumpKey(const char *aPrefix, EKey * Key)
{
    ...
    GetKeyName(KeyName + strlen(KeyName), sizeof(KeyName) - strlen(KeyName),
	       Key->fKey);
	...
	if (Macros[id].cmds[i].type == CT_COMMAND) {
		...
		sprintf(p, "%d:%s ", Macros[id].cmds[i].repeat,
			GetCommandName(Macros[id].cmds[i].u.num));
	    else
...
void EventMapView::DumpMap(const char *aPrefix, EKeyMap * aKeyMap)
{
	...
	if (Key->fKeyMap) {
	    ...
	    GetKeyName(Prefix + strlen(Prefix),
		       sizeof(Prefix) - strlen(Prefix), Key->fKey);
...
EventMapView::EventMapView(int createFlags, EModel ** ARoot,
			   EEventMap * Map):EList(createFlags, ARoot,
						  "Event Map")
{
    BCount = 0;
    BList = 0;
...
void EventMapView::DrawLine(PCell B, int Line, int Col, ChColor color,
			    int Width)
{
    if (Line < BCount)
