...
ECvsLog::ECvsLog(int createFlags, EModel ** ARoot, char *Directory,
		 char *OnFiles):EBuffer(createFlags, ARoot, NULL)
{
    ...
    InsertLine(1, 60,
	       "CVS: -------------------------------------------------------");
    InsertLine(2, 59,
	       "CVS: Enter log. Lines beginning with 'CVS:' will be removed");
    InsertLine(3, 4, "CVS:");
...
void ECvsLog::ListFiles(int &p, const int fCount, const char *title,
...
int ECvsLog::ConfQuit(GxView * V, int /*multiFile */ )
{
    ...
    switch (V->
	    Choice(GPC_ERROR, "CVS commit pending", 3, "C&ommit", "&Discard",
		   "&Cancel", "")) {
    case 0:				// Commit
	...
	for (i = 0; i < RCount;) {
	    ...
	    if (l->Count >= 4 && strncmp(l->Chars, "CVS:", 4) == 0)
		DelLine(i);
	    else
		i++;
	}
...
{
    sprintf(AInfo, "%2d %04d:%03d%cCVS log: %-140s", ModelNo, 1 + CP.Row,
