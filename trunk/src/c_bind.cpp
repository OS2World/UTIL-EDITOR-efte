...
const char *GetCommandName(int Command)
{
    ...
    for (int i = 0;
	 i < int (sizeof(Command_Table) / sizeof(Command_Table[0])); i++)
...
void EMode::AddIndentRx(int look_line, int affect_line, int indent,
...
int GetKeyName(char *Key, int KeySize, KeySel & ks)
{
	...
	strlcat(Key, "C-", KeySize);
    if (ks.Key & kfGray)
	strlcat(Key, "G+", KeySize);
    if (ks.Mask & kfGray)
	strlcat(Key, "G-", KeySize);
    if (ks.Key & kfShift)
	strlcat(Key, "S+", KeySize);
    if (ks.Mask & kfShift)
	strlcat(Key, "S-", KeySize);
...
int AddCommand(int no, int Command, int count, int ign)
{
	...
	return 0;
    Macros[no].cmds =
	(CommandType *) realloc(Macros[no].cmds,
				sizeof(CommandType) * (Macros[no].Count + 1));
...
{
    assert(maxlen >= 0);
    if (Macro == -1 || Pos == -1 || Pos >= Macros[Macro].Count)
	return 0;
