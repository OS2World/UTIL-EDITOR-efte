...
int EBuffer::SetBB(EPoint M)
{
    ...
    if (OldBB.Row == -1)
	OldBB = BE;
    if ((OldBB.Col != BB.Col) && (BlockMode == bmColumn))
	BlockRedraw();
...
int EBuffer::SetBE(EPoint M)
{
    ...
    if (OldBE.Row == -1)
	OldBE = BB;
    if ((OldBE.Col != BE.Col) && (BlockMode == bmColumn))
	BlockRedraw();
...
int EBuffer::CheckBlock()
{
	...
	if (BB.Row > BE.Row)
	    return 0;
	if (BB.Row == BE.Row && BB.Col >= BE.Col)
	    return 0;
	break;
    }
    return 1;
...
int EBuffer::BlockCopy(int Append, int clipboard)
{
	...
	for (L = B.Row; L < E.Row; L++) {
	    if (SSBuffer->InsLine(SL, 0) == 0)
		return 0;
	    if (SSBuffer->InsLineText(SL, 0, -1, 0, RLine(L)) == 0)
		return 0;
	    SL++;
	}
	break;
	...
	for (L = B.Row; L < E.Row; L++) {
		...
		return 0;
	    if (SSBuffer->
		InsLineText(SL, 0, E.Col - B.Col, B.Col, RLine(L)) == 0)
		return 0;
	    if (SSBuffer->PadLine(SL, E.Col - B.Col) == 0)
		return 0;
	    SL++;
	}
	break;
	...
	if (B.Row == E.Row) {
		...
		return 0;
	    if (SSBuffer->
		InsLineText(SL, 0, E.Col - B.Col, B.Col, RLine(B.Row)) == 0)
		return 0;
	}
	else {
	    if (SSBuffer->InsLine(SL, 0) == 0)
		return 0;
	    if (SSBuffer->InsLineText(SL, 0, -1, B.Col, RLine(B.Row)) == 0)
		return 0;
	    SL++;
	    ...
	    if (SSBuffer->InsLine(SL, 0) == 0)
		return 0;
	    if (SSBuffer->InsLineText(SL, 0, E.Col, 0, RLine(E.Row)) == 0)
		return 0;
	}
...
int EBuffer::BlockPaste(int clipboard)
{
	...
	for (L = 0; L < SSBuffer->RCount; L++) {
		...
		return 0;
	    if (InsLineText
		(BL, 0, SSBuffer->LineLen(L), 0, SSBuffer->RLine(L)) == 0)
		return 0;
	    BL++;
	}
	E.Row = BL;
	...
	for (L = 0; L < SSBuffer->RCount; L++) {
		...
		return 0;
	    if (InsLineText
		(BL, B.Col, SSBuffer->LineLen(L), 0, SSBuffer->RLine(L)) == 0)
		return 0;
	    if (TrimLine(BL) == 0)
		return 0;
	    BL++;
	...
	if (SSBuffer->RCount > 1)
	    if (SplitLine(B.Row, B.Col) == 0)
		return 0;
	if (InsLineText
	    (B.Row, B.Col, SSBuffer->LineLen(0), 0, SSBuffer->RLine(0)) == 0)
	    return 0;
	E = B;
...
int EBuffer::BlockKill()
{
	...
	if (Y >= B.Row) {
	    if (Y >= E.Row) {
		if (SetPosR(CP.Col, Y - (E.Row - B.Row)) == 0)
		    return 0;
	    }
	    else {
		if (SetPosR(CP.Col, B.Row) == 0)
		    return 0;
	    }
	}
	for (L = B.Row; L < E.Row; L++)
	...
	if (Y >= B.Row && Y < E.Row) {
	    if (CP.Col >= B.Col) {
		if (CP.Col >= E.Col) {
		    if (SetPos(CP.Col - (E.Col - B.Col), CP.Row) == 0)
			return 0;
		}
		else {
		    if (SetPos(B.Col, CP.Row) == 0)
			return 0;
		}
	    }
	}
	for (L = B.Row; L < E.Row; L++)
	    if (DelText(L, B.Col, E.Col - B.Col) == 0)
		return 0;
	break;
	...
	if (B.Row == E.Row) {
	    ...
	    if (DelText(B.Row, B.Col, E.Col - B.Col) == 0)
		return 0;
	}
	else {
	    if (Y >= B.Row) {
		if (Y > E.Row || (Y == E.Row && E.Col == 0)) {
		    ...
		    if (CP.Col >= E.Col) {
			if (SetPosR(CP.Col - E.Col + B.Col, B.Row) == 0)
			    return 0;
		    }
		    else {
			if (SetPosR(B.Col, B.Row) == 0)
			    return 0;
		    }
		}
		else {
	    ...
	    if (DelText(E.Row, 0, E.Col) == 0)
		return 0;
	    for (L = B.Row + 1; L < E.Row; L++)
		if (DelLine(B.Row + 1) == 0)
		    return 0;
	    if (DelText(B.Row, B.Col, -1) == 0)
		return 0;
	    if (JoinLine(B.Row, B.Col) == 0)
		return 0;
	}
	break;
    }
...
int EBuffer::BlockIndent()
{
	...
	case bmColumn:
	    if (L < E.Row) {
		if (InsText(L, B.Col, 1, 0) == 0)
		    return 0;
		if (DelText(L, E.Col, 1) == 0)
		    return 0;
	    }
	    break;
	}
...
int EBuffer::BlockUnindent()
{
	...
	case bmColumn:
	    if (L < E.Row) {
		if (InsText(L, E.Col, 1, 0) == 0)
		    return 0;
		if (DelText(L, B.Col, 1) == 0)
		    return 0;
	    }
	    break;
	}
...
int EBuffer::BlockExtendBegin()
{
    ...
    switch (BlockMode) {
    case bmStream:
	if ((Y == BB.Row) && (CP.Col == BB.Col))
	    ExtendGrab |= 1;
	if ((Y == BE.Row) && (CP.Col == BE.Col))
	    ExtendGrab |= 2;
	break;
    case bmLine:
	if (Y == BB.Row)
...
{
    if ((BB.Row != -1) && (BE.Row != -1) && (BB.Col != -1) && (BE.Col != -1))
	return 1;
    return 0;
}
...
int EBuffer::BlockSelectWord()
{
	...
	P--;
    if (SetBB(EPoint(Y, ScreenPos(L, P))) == 0)
	return 0;
    while ((P < L->Count) && (C == ChClassK(L->Chars[P])))
	P++;
    if (SetBE(EPoint(Y, ScreenPos(L, P))) == 0)
	return 0;
    return 1;
}
...
int EBuffer::BlockSelectLine()
{
    ...
    if (SetBB(EPoint(Y, 0)) == 0)
	return 0;
    if (Y == RCount - 1) {
	if (SetBE(EPoint(Y, LineLen(Y))) == 0)
	    return 0;
    }
    else {
	if (SetBE(EPoint(Y + 1, 0)) == 0)
	    return 0;
    }
    return 1;
}
...
int EBuffer::BlockWriteTo(const char *AFileName, int Append)
{
	...
	if (A != -1 && Z != -1) {
	    ...
	    if (bc > 65536 + oldc) {
		Msg(S_INFO, "Writing %s, %d lines, %d bytes.", AFileName, lc,
		    bc);
		oldc = bc;
	    }
	}
  ...
  error:
    ...
    View->MView->Win->Choice(GPC_ERROR, "Error", 1, "O&K",
			     "Failed to write block to %s", AFileName);
    return 0;
}
...
static int _LNK_CONV SortProc(const void *A, const void *B)
{
	...
	if (lA < 0 && lB < 0)
	    ...
	    if (BFI(SortBuffer, BFI_MatchCase) == 1)
		rc = memcmp(LA->Chars + PA, LB->Chars + PB,
			    (lA < lB) ? lA : lB);
	    else
		rc = memicmp(LA->Chars + PA, LB->Chars + PB,
			     (lA < lB) ? lA : lB);
	    if (rc == 0) {
		if (lA > lB)
		    rc = 1;
...
int EBuffer::BlockEnTab()
{
	...
	while (O < L->Count) {
	    ...
	    else if (((C % BFI(this, BFI_TabSize)) == (BFI(this, BFI_TabSize) - 1)) && (C - C1 > 0)) {	// reached a tab and can put one
