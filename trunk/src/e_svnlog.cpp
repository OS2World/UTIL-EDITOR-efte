...
ESvnLog::ESvnLog(int createFlags, EModel ** ARoot, char *Directory,
		 char *OnFiles):EBuffer(createFlags, ARoot, NULL)
{
    int i, j, p;
    ...
    InsertLine(1, 60,
	       "SVN: -------------------------------------------------------");
    InsertLine(2, 59,
	       "SVN: Enter log. Lines beginning with 'SVN:' will be removed");
    InsertLine(3, 4, "SVN:");
	...
	ListFiles(p, fAdded, "Added", cnt, position, len, status, OnFiles,
		  "Aa");
	ListFiles(p, fRemoved, "Removed", cnt, position, len, status, OnFiles,
		  "Rr");
	ListFiles(p, fModified, "Modified", cnt, position, len, status,
		  OnFiles, "Mm");
	ListFiles(p, fOther, "Other", cnt, position, len, status, OnFiles,
		  "AaRrMm", 1);
	delete[]position;
	delete[]len;
	delete[]status;
...
void ESvnLog::ListFiles(int &p, const int fCount, const char *title,
			const int cnt, const int *position, const int *len,
			const char *status, const char *list,
			const char *excinc, const int exc)
{
    if (fCount) {
...
int ESvnLog::ConfQuit(GxView * V, int /*multiFile */ )
{
    ...
    switch (V->
	    Choice(GPC_ERROR, "SVN commit pending", 3, "C&ommit", "&Discard",
		   "&Cancel", "")) {
    case 0:				// Commit
	...
	for (i = 0; i < RCount;) {
	    ...
	    if (l->Count >= 4 && strncmp(l->Chars, "SVN:", 4) == 0)
		DelLine(i);
	    else
		i++;
	}
...
{
    sprintf(AInfo, "%2d %04d:%03d%cSVN log: %-140s", ModelNo, 1 + CP.Row,
	    1 + CP.Col, Modified ? '*' : ' ', FileName);
}
