...
int ParseSearchOptions(int replace, const char *str, unsigned long &Options)
{
    ...
    Options = SEARCH_NEXT;
    while (*p) {
	if (ParseSearchOption(replace, *p, Options) == 0)
	    return 0;
	p++;
    }
    return 1;
...
int ParseSearchReplace(EBuffer * B, const char *str, int replace,
		       SearchReplaceOptions & opt)
{
	...
	return 0;
    if (replace) {
	if (ParseSearchOptions
	    (replace, BFS(B, BFS_DefFindReplaceOpt), opt.Options) == 0)
	    return 0;
	opt.Options |= SEARCH_REPLACE;
    }
    else {
	if (ParseSearchOptions(replace, BFS(B, BFS_DefFindOpt), opt.Options)
	    == 0)
	    return 0;
    }
	...
	default:
		...
		p++;
		if (ParseSearchOption(replace, c, opt.Options) == 0)
		    return 0;
	    }
	}
    }
	...
	if (UnquoteString(opt.strSearch) == 0)
	    return 0;
	if (opt.Options & SEARCH_REPLACE)
	    if (UnquoteString(opt.strReplace) == 0)
		return 0;
    }
...
int EBuffer::FindStr(char *Data, int Len, SearchReplaceOptions & opt)
{
	...
	Forward = 1;
    if (Options & SEARCH_ALL && Options & SEARCH_BACK
	...
	if (Options & SEARCH_BLOCK) {
	    if (BlockMode == bmStream) {
		if (L > BE.Row || L < BB.Row)
		    break;
	    }
	    else if (L >= BE.Row || L < BB.Row)
		break;
	}
	else if (L >= RCount || L < 0)
	    break;
	...
	while (((!(Options & SEARCH_BACK)) && (C <= End - Len)) ||
		...
		if (Options & SEARCH_REPLACE && !(Options & SEARCH_BACK)) {
		    ...
		    else if (Match.Col == 0 && Match.Row == 0
			     && (Options & SEARCH_BACK)) {
			C = L = 0;
		    }
		    else {
...
int EBuffer::FindRx(RxNode * Rx, SearchReplaceOptions & opt)
{
	...
	View->MView->Win->Choice(GPC_ERROR, "FindRx", 1, "O&K",
				 "Reverse regexp search not supported.");
	return 0;
    }
    if (Rx == 0)
	return 0;
...
int EBuffer::Find(SearchReplaceOptions & opt)
{
	...
	if (R == 0) {
	    View->MView->Win->Choice(GPC_ERROR, "Find", 1, "O&K",
	...
	if (Options & SEARCH_BLOCK) {
	    if (Options & SEARCH_BACK) {
		if (SetPosR(BE.Col, BE.Row) == 0)
		    goto error;
	    }
	    else {
		if (SetPosR(BB.Col, BB.Row) == 0)
		    goto error;
	    }
	}
	else {
		...
		if (RCount < 1)
		    goto error;
		if (SetPosR(LineLen(RCount - 1), RCount - 1) == 0)
		    goto error;
	    }
	    else {
		if (SetPosR(0, 0) == 0)
	...
	if (Options & SEARCH_RE) {
	    ...
	    if (FindStr(opt.strSearch, slen, opt) == 0)
		goto end;
	}
	opt.resCount++;
	...
	if (opt.Options & SEARCH_REPLACE) {
		...
		if (0 ==
		    RxReplace(opt.strReplace, L->Chars, L->Count, MatchRes,
			      &PR, &LR)) {
		    if (DelText(R, Match.Col, MatchLen) == 0)
			goto error;
		    if (PR && LR > 0)
			if (InsText(R, Match.Col, LR, PR) == 0)
			    goto error;
		    if (PR)
			free(PR);
		    rlen = LR;
		...
		if (DelText(Match.Row, Match.Col, MatchLen) == 0)
		    goto error;
		if (InsText(Match.Row, Match.Col, rlen, opt.strReplace) == 0)
		    goto error;
	...
	if (Options & SEARCH_JOIN) {
	    ...
	    if (JoinLine(Match.Row, Match.Col) == 0)
		goto error;
	...
	if (Options & SEARCH_DELETE) {
		...
		if (DelText(Match.Row, 0, LineLen()) == 0)
		    goto error;
	    }
	    else if (DelLine(Match.Row) == 0)
		goto error;
...
int EBuffer::Search(ExState & State, char *aString, int Options,
		    int /*CanResume */ )
{
    ...
    if (aString)
	strcpy(find, aString);
    else if (State.GetStrParam(View, find, sizeof(find)) == 0)
	if ((erc =
	     View->MView->Win->GetStr("Find", sizeof(find), find,
				      HIST_SEARCH)) == 0)
	    return 0;
    if (strlen(find) == 0)
	return 0;
...
int EBuffer::SearchReplace(ExState & State, char *aString,
			   char *aReplaceString, int Options)
{
    char find[MAXSEARCH + 1] = "";
	...
	if (View->MView->Win->
	    GetStr("Find", sizeof(find), find, HIST_SEARCH) == 0)
	    return 0;
    if (strlen(find) == 0)
	return 0;
    if (aReplaceString)
	strcpy(replace, aReplaceString);
    else if (State.GetStrParam(View, replace, sizeof(replace)) == 0)
	if (View->MView->Win->
	    GetStr("Replace", sizeof(replace), replace, HIST_SEARCH) == 0)
	    return 0;
    ...
    LSearch.Options =
	Case | (Options & ~SEARCH_NCASE) | SEARCH_ALL | SEARCH_REPLACE;
    LSearch.ok = 1;
    if (Find(LSearch) == 0)
	return 0;
    return 1;
...
int EBuffer::ScanForRoutines()
{
    ...
    if (regx == 0) {
	View->MView->Win->Choice(GPC_ERROR, "Error", 1, "O&K",
				 "Failed to compile regexp '%s'", BFS(this,
								      BFS_RoutineRegexp));
	return 0;
    }
	...
	if (RxExec(regx, L->Chars, L->Count, L->Chars, &res) == 1) {
	    rlst.Count++;
	    rlst.Lines =
		(int *)realloc((void *)rlst.Lines,
			       sizeof(int) * (rlst.Count | 0x1F));
...
int EBuffer::ShowPosition()
{
...
#ifdef HEAPWALK
    unsigned long MemUsed = 0, MemFree = 0, BlkUsed = 0, BlkFree =
	0, BigFree = 0, BigUsed = 0;
#endif
...
#endif
	"L%d/%d G%d/%d/%d A%d/%d C%d/%d P%d/%d " "U%d/%d/%d " "H%d/%d/%d",
#ifdef HEAPWALK
	...
	CColumn, NColumns,
	CCharPos, NChars,
	US.UndoPtr, US.Num, NN, StartHilit, MinRedraw, MaxRedraw);
    return 1;
}
...
int EBuffer::RemoveBookmark(char *Name)
{
	...
	if (strcmp(Name, BMarks[i].Name) == 0) {
	    free(BMarks[i].Name);
	    memmove(BMarks + i, BMarks + i + 1,
		    sizeof(EBookmark) * (BMCount - i - 1));
	    BMCount--;
	    BMarks =
		(EBookmark *) realloc(BMarks, sizeof(EBookmark) * BMCount);
	    return 1;
	}
    }
    View->MView->Win->Choice(GPC_ERROR, "RemoveBookmark", 1, "O&K",
			     "Bookmark %s not found.", Name);
    return 0;
}
 ...
 * bookmarks.
 */
int EBuffer::GetBookmarkForLine(int searchFrom, int searchForLine,
				char *&Name, EPoint & P)
{
...
int EBuffer::GotoBookmark(char *Name)
{
    ...
    View->MView->Win->Choice(GPC_ERROR, "GotoBookmark", 1, "O&K",
			     "Bookmark %s not found.", Name);
    return 0;
}
...
int EBuffer::GetMatchBrace(EPoint & M, int MinLine, int MaxLine, int show)
{
    ...
    StateMap = 0;
    if (GetMap(M.Row, &StateLen, &StateMap) == 0)
	return 0;
    State = StateMap[Pos];
    StateRow = M.Row;
...
int EBuffer::SearchWord(int SearchFlags)
{
    ...
    while ((P > 0)
	   && ((ChClass(L->Chars[P - 1]) == 1) || (L->Chars[P - 1] == '_')))
	P--;
    while (len < int (sizeof(word)) && P < L->Count
	   && (ChClass(L->Chars[P]) == 1 || L->Chars[P] == '_'))
	word[len++] = L->Chars[P++];
    word[len] = 0;
    if (len == 0)
	return 0;
