...
int Hilit_C(EBuffer * BF, int /*LN*/, PCell B, int Pos, int Width,
...
int LookAt(EBuffer * B, int Row, unsigned int Pos, const char *What,
	   hsState State, int NoWord, int CaseInsensitive)
{
	...
	if (B->GetMap(Row, &StateLen, &StateMap) == 0)
	    ENDFUNCRC(0);
	if (IsState(StateMap + Pos, State, strlen(What)))
	    ENDFUNCRC(1);
    }
    ENDFUNCRC(0);
}
...
static int CheckLabel(EBuffer * B, int Line)
{
    ...
    while ((P < L->Count) && (L->Chars[P] == ' ' || L->Chars[P] == 9))
	P++;
	...
	if (Cnt > 0)
		...
		return 1;
	if (!isalnum(L->Chars[P]) && L->Chars[P] != '_')
	    return 0;
	Cnt++;
	P++;
    }
...
static int SearchBackMatch(int Count, EBuffer * B, int Row, hsState State,
			   const char *Open, const char *Close, int *OPos,
			   int *OLine, int matchparens = 0, int bolOnly = 0)
{
    char *P;
    int L;
    int Pos;
    ...
    *OPos = 0;
	...
	if (L > 0)
		...
		if (P[Pos] != ' ' && P[Pos] != 9) {
		    ...
		    if (!matchparens ||
			(CountX[0] == 0 && CountX[1] == 0
			 && CountX[2] == 0)) {
			if (LOpen + Pos <= L) {
			    if (IsState(StateMap + Pos, State, LOpen)) {
				if (memcmp(P + Pos, Open, LOpen) == 0)
				    Count++;
				if (Count == 0) {
				    if (bolOnly)
					didMatch = 1;
				...
				if (IsState(StateMap + Pos, State, LClose)) {
				    if (memcmp(P + Pos, Close, LClose) == 0)
					Count--;
				}
			    }
			}
...
static int FindPrevIndent(EBuffer * B, int &RowP, int &ColP, char &CharP,
			  int Flags)
{
	...
	if (L > 0)
	    while (ColP >= 0) {
		LOG << "ColP: " << ColP << " State: " << (int)StateMap[ColP]
		    << ENDLINE;
		if (StateMap[ColP] == hsC_Normal) {
		    LOG << "CharP: " << BinChar(P[ColP]) << " BolChar: " <<
			BinChar(BolChar) << " BolRow: " << BolRow <<
			" BolCol: " << BolCol << ENDLINE;
		    switch (CharP = P[ColP]) {
		    case '{':
			if (BolChar == ':' || BolChar == ',') {
			    CharP = BolChar;
		...
		else if (StateMap[ColP] == hsC_Keyword
			 && (BolChar == ' ' || BolChar == ':')) {
		    if (L - ColP >= 2
...
static int IndentNormal(EBuffer * B, int Line, int /*StateLen */ ,
			hsState * /*StateMap */ )
{
	...
	I = SearchBackMatch(-1, B, Line - 1, hsC_Normal, "{", "}", &Pos,
			    &L) + C_CASE_OFS;
	return I;
    }
	...
	I = SearchBackMatch(-1, B, Line - 1, hsC_Keyword, "if", "else", &Pos,
			    &L, 1);
	return I;
    }
	...
	PrevColP++;
	LOG << "PrevRowP=" << PrevRowP << ", PrevColP=" << PrevColP <<
	    ENDLINE;

	if (FindPrevIndent(B, RowP, ColP, CharP,
			   FIND_IF |
			   FIND_ELSE |
			   FIND_FOR | FIND_WHILE | FIND_SWITCH | FIND_CASE |
			   //FIND_CLASS |
			   FIND_COLON |
			   FIND_SEMICOLON |
	...
	case '[':
		...
		ColP++;
		if (SkipWhite(B, Line, RowP, ColP, SKIP_FORWARD | SKIP_LINE)
		    != 1)
		    return 0;
		if (ColP < B->LineChars(RowP) || !FunctionUsesContinuation) {
		    ...
		    I = SearchBackMatch(-1, B, Line - 1, hsC_Normal, strLeft,
					strRight, &Pos, &L);
		    I = Pos + 1;
		}
		else {
	...
	case '{':
	    ...
	    if (SkipWhite
		(B, Line, RowP, ColP,
		 SKIP_BACK | SKIP_TOBOL | SKIP_MATCH) != 1)
		return 0;
	...
	case ':':
		...
		if (CheckLabel(B, RowP))
		    I -= C_COLON_OFS;
		else if (PrevRowP == RowP && FirstRowP == PrevRowP
			 && FirstColP + 1 == PrevColP)
		    I += C_CONTINUATION;
		if (LookAt(B, Line, 0, "{", hsC_Normal, 0)
		    && LookAt(B, RowP, ColP, "{", hsC_Normal, 0))
		    I -= 0;		//C_BRACE_OFS;
	...
	case 'w':
	    ...
	    if (SkipWhite(B, Line, RowP, ColP,
			  SKIP_FORWARD | (CharP != 'e' ? SKIP_MATCH : 0)) !=
		1)
		return 0;
	    if (RowP >= Line) {
		RowP = Line;
...
static int IndentComment(EBuffer * B, int Row, int /*StateLen */ ,
			 hsState * /*StateMap */ )
{
    int I = 0, R;
	...
	if (B->RLine(Row - 1)->StateE == hsC_Comment)
	    if (LookAt(B, Row - 1, I, "/*", hsC_Comment, 0))
		I += C_COMMENT_DELTA;
	if (B->RLine(Row - 1)->StateE == hsC_CPP_Comm)
	    if (LookAt(B, Row - 1, I, "/*", hsC_CPP_Comm, 0))
		I += C_COMMENT_DELTA;
    }
    return I;
}

static int IndentCPP(EBuffer * B, int Line, int /*StateLen */ ,
		     hsState * /*StateMap */ )
{
...
int Indent_C(EBuffer * B, int Line, int PosCursor)
{
	...
	default:
		...
		if (StateMap[0] == hsC_CPP || StateMap[0] == hsC_CPP_Comm ||
		    StateMap[0] == hsC_CPP_String1
		    || StateMap[0] == hsC_CPP_String2
			...
			if (LookAt(B, Line, 0, "case", hsC_Keyword) ||
			    ...
			    LookAt(B, Line, 0, "private:", hsC_Keyword, 0) ||
			    LookAt(B, Line, 0, "protected:", hsC_Keyword,
				   0)) ;
			else
			    I += C_COLON_OFS;
		    }			//else if (LookAt(B, Line, 0, "{", hsC_Normal, 0)) {
