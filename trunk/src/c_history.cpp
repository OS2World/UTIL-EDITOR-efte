...
{					/*FOLD00 */
	...
	for (i = 0; i < FPHistoryCount; i++) {
	    fprintf(fp, "F|%d|%d|%s\n",
		    FPHistory[i]->Row,
		    FPHistory[i]->Col, FPHistory[i]->FileName);
...
{					/*FOLD00 */
    ...
    if (fgets(line, sizeof(line), fp) == 0 || strcmp(line, HISTORY_VER) != 0) {
	fclose(fp);
	return 0;
    }
	...
	if (line[0] == 'F' && line[1] == '|') {	// file position history
	    ...
	    *e = 0;
		...
		if (e == 0)
		    break;
		*e = 0;
		nb = (HBookmark **) realloc(last->Books,
					    sizeof(HBookmark *) *
					    (last->BookCount + 1));
		if (nb) {
		    last->Books = nb;
		    nb[last->BookCount] =
			(HBookmark *) malloc(sizeof(HBookmark));
...
{					/*FOLD00 */
    ...
    N = 64;
    while (N <= FPHistoryCount)
	N *= 2;
    NH = (FPosHistory **) realloc((void *)FPHistory,
				  N * sizeof(FPosHistory *));
    if (NH == 0) {
	free(fp->FileName);
	free(fp);
	return 0;
	...
	memmove(FPHistory + L + 1,
		FPHistory + L, (FPHistoryCount - L) * sizeof(FPosHistory *));
    FPHistory[L] = fp;
    FPHistoryCount++;
    return 1;
}
...
{					/*FOLD00 */
	...
	if (inputHistory.Id[i] == Id
	    && strcmp(String, inputHistory.Line[i]) == 0) {
	...
	if (inputHistory.Count < MAX_INPUT_HIST) {
	    ...
	    inputHistory.Line = (char **)realloc((void *)inputHistory.Line,
						 ...
						 sizeof(char *));
	    inputHistory.Id =
		(int *)realloc((void *)inputHistory.Id,
 ...
 */
int RetrieveBookmarks(EBuffer * buffer)
{					/*FOLD00 */
	...
	if (cmp == 0) {
		...
		if (P.Row < 0)
		    P.Row = 0;
		else if (P.Row >= buffer->RCount)
		    P.Row = buffer->RCount - 1;
		if (P.Col < 0)
		    P.Col = 0;
 ...
 */
int StoreBookmarks(EBuffer * buffer)
{					/*FOLD00 */
	...
	if (UpdateFPos(buffer->FileName, 0, 0) == 0)
	    return 0;
    }
	...
	if (cmp == 0) {
	    ...
	    for (i = j = 0;
		 (i = buffer->GetUserBookmarkForLine(i, -1, name, P)) >= 0;
		 j++) ;
	    FPHistory[M]->BookCount = j;
	    if (j) {
		// Something to store
		FPHistory[M]->Books =
		    (HBookmark **) malloc(sizeof(HBookmark *) * j);
		if (FPHistory[M]->Books) {
		    for (i = j = 0;
			 (i =
			  buffer->GetUserBookmarkForLine(i, -1, name,
							 P)) >= 0; j++) {
			bmk = FPHistory[M]->Books[j] =
			    (HBookmark *) malloc(sizeof(HBookmark));
			if (bmk) {
			    bmk->Row = P.Row;
			    bmk->Col = P.Col;
