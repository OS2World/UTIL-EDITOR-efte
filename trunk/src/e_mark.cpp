...
int EMark::removeBuffer(EBuffer * aBuffer)
{
    ...
    if (!Buffer->GetBookmark(Name, Point))
	return 0;
...
EMark *EMarkIndex::insert(char *aName, char *aFileName, EPoint aPoint,
			  EBuffer * aBuffer)
{
...
int EMarkIndex::remove(char *aName)
{
	...
	if (cmp == 0) {
	    ...
	    memmove(marks + M,
		    marks + M + 1, sizeof(marks[0]) * (markCount - M - 1));
	    markCount--;
...
int EMarkIndex::saveToDesktop(FILE * fp)
{
	...
	fprintf(fp, "M|%d|%d|%s|%s\n",
		p.Row, p.Col, marks[n]->getName(), marks[n]->getFileName());
    }
    return 1;
}
