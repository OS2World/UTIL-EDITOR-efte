...
int Hilit_REXX(EBuffer * BF, int /*LN*/, PCell B, int Pos, int Width,
...
static int Match2(int Len, int Pos, hsState * StateMap, const char *Text,
		  const char *String, hsState State)
{
    int L = strlen(String);
	...
	Pos = L - 1;
		...
		if (isalpha(P[Pos])) {
			...
			if (Match(L, Pos, StateMap, P, "do", hsREXX_Keyword)
			    || Match(L, Pos, StateMap, P, "loop",
				     hsREXX_Keyword)
			    || Match(L, Pos, StateMap, P, "select",
				     hsREXX_Keyword)) {
			    Count++;
			    if (Count == 0) {
				if (StateMap)
			...
			if (Match2
			    (L, Pos, StateMap, P, "class", hsREXX_Keyword)) {
			    if (StateMap)
				free(StateMap);
	...
	if (L > 0)
		...
		if (isalpha(P[Pos]) && (Pos == 0 || !isalpha(P[Pos - 1]))) {
			...
			//ChFind = 'p';
			     ...
			     (L, Pos, StateMap, P, "select",
			      hsREXX_Keyword)) {
			Count++;
			ChFind = 's';
		    }
		    else if (Match
			     (L, Pos, StateMap, P, "method",
			      hsREXX_Keyword)) {
			Count++;
			ChFind = 'm';
		    }
		    else if (Match
			     (L, Pos, StateMap, P, "properties",
			      hsREXX_Keyword)) {
			Count++;
			ChFind = 'r';
		    }
			...
			return B->LineIndented(Row) + REXX_BASE_INDENT;
			     ...
			     (L, Pos, StateMap, P, "otherwise",
			      hsREXX_Keyword) && Count == 0) {
			//Count++;
			ChFind = 'o';
			if (StateMap)
			    free(StateMap);
			return B->LineIndented(Row);
			     ...
			     (L, Pos, StateMap, P, "end", hsREXX_Keyword)) {
			Count--;
			ChFind = 'e';
		    }
		    else if (is_blank < 5
			     && Match(L, Pos, StateMap, P, "then",
				      hsREXX_Keyword)) {
			ChFind = 't';
			if (StateMap)
			    free(StateMap);
			...
			hsState * /*StateMap */ )
{
	...
	return SearchMatch(-1, B, Line - 1, 3);
	     ...
	     || LookAtNoCase(B, Line, 0, "properties", hsREXX_Keyword)) {
