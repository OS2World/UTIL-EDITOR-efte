...
int GViewPeer::ExpandHeight(int DeltaY)
{
    ...
    View->Next->Peer->ConSetSize(View->Next->Peer->wW,
...
int GViewPeer::DrawScrollBar()
{
    ...
    if (ShowVScroll) {
	MoveCh(B, ConGetDrawChar(DCH_AUP), (TAttr) hcScrollBar_Arrows, 1);
	ConPutBox(W, 0, 1, 1, B);
	MoveCh(B, ConGetDrawChar(DCH_ADOWN), (TAttr) hcScrollBar_Arrows, 1);
	...
	for (I = 0; I < NRows; I++) {
		...
		MoveCh(B, fore, (TAttr) hcScrollBar_Fore, 1);
	    else
		MoveCh(B, back, (TAttr) hcScrollBar_Back, 1);
	    ConPutBox(W, I + 1, 1, 1, B);
	}
    }
    if (ShowHScroll) {
	MoveCh(B, ConGetDrawChar(DCH_ALEFT), (TAttr) hcScrollBar_Arrows, 1);
	ConPutBox(0, H, 1, 1, B);
	MoveCh(B, ConGetDrawChar(DCH_ARIGHT), (TAttr) hcScrollBar_Arrows, 1);
	...
	MoveCh(B, ' ', (TAttr) hcScrollBar_Arrows, 1);
	ConPutBox(W, H, 1, 1, B);
    }
...
void GFrame::Update()
{
	...
	v->Update();
	if ((ShowVScroll || ShowHScroll)
	    && (v->Peer->sbVupdate || v->Peer->sbHupdate)) {
...
static void HandleVScroll(GView * view, TEvent & E)
{
	...
	do {
	    ...
	    E1.Msg.Param1 =
		(E.Mouse.Y - wY - 1 - delta +
		 1) * view->Peer->sbVtotal / (wH - 2);
...
static void HandleHScroll(GView * view, TEvent & E)
{
	...
	do {
	    ...
	    E1.Msg.Param1 =
		(E.Mouse.X - wX - 1 - delta +
		 1) * view->Peer->sbHtotal / (wW - 2);
//            printf("YPos = %d %d %d \n\x7", E.Mouse.Y, wY, delta);
	    gui->DispatchEvent(frames, view, E1);
	    frames->Update();
	    do {
...
void GUI::ProcessEvent()
{
	...
	while (ConGetEvent(evMouse | evCommand | evKeyboard, &E, -1, 1, 0) ==
	       -1 || (E.What == evMouseMove && E.Mouse.Buttons == 0)) ;
    }
    if (E.What != evNone) {
...
//                fprintf(stderr, "Command got = %d\n", E.Msg.Command);
	    ...
	    if (E.What == evMouseDown && MouseCapture == 0
		&& FocusCapture == 0) {
		...
		while (V) {
			...
			E.Mouse.Y <
			V->Peer->wY + V->Peer->wH + (ShowHScroll ? 1 : 0)) {
			frames->SelectView(V);
			view = V;
			break;
		    }
		...
		if (ShowVScroll && E.What == evMouseDown && MouseCapture == 0
		    && FocusCapture == 0
		    && E.Mouse.X == view->Peer->wX + view->Peer->wW) {
		    HandleVScroll(view, E);
		    return;
		}
		if (ShowHScroll && E.What == evMouseDown && MouseCapture == 0
		    && FocusCapture == 0
		    && E.Mouse.Y == view->Peer->wY + view->Peer->wH) {
