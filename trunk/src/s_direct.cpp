...
int FileFind::FindFirst(FileInfo ** fi)
{
    ...
    if ((rc = DosFindFirst(fullpattern,
			   &hdir,
			   attr,
			   &find, sizeof(find), &count, FIL_STANDARD)) != 0) {
	//fprintf(stderr, "%s: %d\n\n", fullpattern, rc);
	return -1;
    }
    dir = hdir;
    ...
    *fi = new FileInfo(name,
		       (find.
			attrFile & FILE_DIRECTORY) ? fiDIRECTORY : fiFILE,
		       find.cbFile, mktime(&t));
    return 0;
#elif defined(NT) && !defined(USE_DIRENT)
    ...
    *fi = new FileInfo(name, (find.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ? fiDIRECTORY : fiFILE, find.nFileSizeLow,	// !!!
		       mktime(&t));
    return 0;
#endif
#endif
...
int FileFind::FindNext(FileInfo ** fi)
{
  ...
  again:
	...
	*fi = new FileInfo(name,
    ...
    if ((rc = DosFindNext(dir, &find, sizeof(find), &count)) != 0) {
	//fprintf(stderr, "%d\n\n", rc);
	return -1;
    }
    if (count != 1)
    ...
    *fi = new FileInfo(name,
    ...
    if ((rc = FindNextFile((HANDLE) dir, &find)) != TRUE) {
	//fprintf(stderr, "%d\n\n", rc);
	return -1;
    }
    ...
    *fi = new FileInfo(name,
			...
			dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) ?
		       fiDIRECTORY : fiFILE, find.nFileSizeLow, mktime(&t));
    return 0;
#endif
    //    put code here
#endif
