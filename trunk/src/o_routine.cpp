...
RoutineView::RoutineView(int createFlags, EModel ** ARoot,
			 EBuffer * AB):EList(createFlags, ARoot, "Routines")
{
    SearchLen = 0;
	...
	sprintf(CTitle, "Routines %s: %d",
		Buffer->FileName, Buffer->rlst.Count);
	SetTitle(CTitle);
    }
}
...
int RoutineView::getMatchingLine(int start, int direction)
{
	...
	for (int j = 0; str[j]; j++) {
	    if (str[j] == SearchString[0]
		&& strnicmp(SearchString, str + j, SearchLen) == 0) {
		return i;
	    }
	}
	i += direction;
...
{
    sprintf(AInfo, "%2d %04d/%03d Routines (%s)", ModelNo, Row + 1, Count,
	    Buffer->FileName);
}

void RoutineView::GetTitle(char *ATitle, int /*MaxLen */ , char *ASTitle,
			   int SMaxLen)
{
