...
#ifdef USE_SCRNMAP
static int conread(int fd, void *p, int len)
{					// len should be a multiple of 2
    char buf[512];
    char *c = (char *)p;
    char *s = buf;
...
static int conwrite(int fd, void *p, int len)
{					// len should be a multiple of 2
    char buf[512];
    char *s = (char *)p;
    char *c = buf;
...
int ConGetTitle(char *Title, int /*MaxLen */ , char * /*STitle */ ,
		int /*SMaxLen */ )
{
    *Title = 0;
    return 0;
...
//            lock_state &= ~(1 << ksv);
    ...
    if (shift_final & ((1 << KG_SHIFT) | (1 << KG_SHIFTL) | (1 << KG_SHIFTR))) {
	shift_final |= (1 << KG_SHIFT);
	shift_final &= ~((1 << KG_SHIFTL) | (1 << KG_SHIFTR));
    }
    if (shift_final & ((1 << KG_CTRL) | (1 << KG_CTRLL) | (1 << KG_CTRLR))) {
	...
	(shift_final & (1 << KG_SHIFTL)) || (shift_final & (1 << KG_SHIFTR)))
	ShiftFlags |= kfShift;
    if ((shift_final & (1 << KG_CTRL)) ||
	(shift_final & (1 << KG_CTRLL)) || (shift_final & (1 << KG_CTRLR)))
	ShiftFlags |= kfCtrl;
...
//        (shift_final & (1 << KG_ALTGR)))
	...
	for (unsigned int i = 0; i < sizeof(KeyTrans) / sizeof(KeyTrans[0]);
	     i++) {
...
//                ShiftFlags &= ~kfAlt;
		...
		for (int i = 0;
		     (unsigned)i < sizeof(DeadTrans) / sizeof(DeadTrans[0]);
		     i++) {
...
int GetMouseEvent(TEvent * Event)
{
	...
	Event->Mouse.Count = (e.type & GPM_SINGLE) ? 1
	    : (e.type & GPM_DOUBLE) ? 2 : (e.type & GPM_TRIPLE) ? 3 : 1;
	Event->Mouse.Buttons =
	...
	if (e.y < 0)
	    e.y = 0;
	if (e.x >= int (VideoCols))
	    e.x = VideoCols - 1;
	if (e.y >= int (VideoRows))
	    e.y = VideoRows - 1;
