...
{
    if (OldLeftCol != LeftCol || OldTopRow != TopRow
	|| OldCount != List->Count)
...
void EListPort::RepaintStatus()
{
    ...
    View->MView->Win->SetSbVPos(TopRow, H,
				List->Count + (WeirdScroll ? H - 1 : 0));
...
EList::EList(int createFlags, EModel ** ARoot,
	     const char *aTitle):EModel(createFlags, ARoot)
{
    Title = strdup(aTitle);
...
void EList::DrawLine(PCell /*B*/, int /*Line */ , int /*Col */ ,
		     ChColor /*color */ , int /*Width */ )
{
}
...
void EList::FixPos()
{
    ...
    if (TopRow + scrollBorderY > Row)
	TopRow = Row - scrollJumpY + 1 - scrollBorderY;
    if (TopRow + H - scrollBorderY <= Row)
	TopRow = Row - H + 1 + scrollJumpY - 1 + scrollBorderY;
    if (TopRow < 0)
	TopRow = 0;
