...
int ReadKbdEvent(TEvent * Event, int Wait)
{
    ...
    if ((Flags & (LEFTSHIFT | RIGHTSHIFT)) != 0)
	KeyFlags |= kfShift;
    if ((Flags & (LEFTCONTROL | RIGHTCONTROL)) != 0)
	KeyFlags |= kfCtrl;
...
int ReadMouseEvent(TEvent * Event, ULONG EventMask)
{
	...
	if (LastMouseEvent.What == evMouseAuto && (EventMask & evMouseAuto)) {
		...
		*Event = LastMouseEvent;
		DosQuerySysInfo(QSV_MS_COUNT, QSV_MS_COUNT, &LastEventTime,
				4);
		return 1;
	    }
	}
    ...
    if (MouReadEventQue(&mi, &fWait, MouseHandle) != 0)
	return 0;
...
int ConPutBox(int X, int Y, int W, int H, PCell Cell)
{
	...
	VioWrtCellStr((PCH) p, (USHORT) (W << 1), (USHORT) (Y + I),
		      (USHORT) X, 0);
		...
		int Delete)
{
	...
	*Event = MouseEv;
	...
	if ((ReadKbdEvent(Event, WaitTime) == 1) && (EventMask & evKeyboard))
	    break;
	else if (MousePresent && (ReadMouseEvent(Event, EventMask) == 1)
		 && (EventMask & evMouse))
	    break;
...
int GUI::OpenPipe(char *Command, EModel * notify)
{
	...
	if (Pipes[i].used == 0) {
	    ...
	    Pipes[i].tid = _beginthread(PipeThread,
					...
					16384, &Pipes[i]);
#else
	    DosCreateThread(Pipes[i].tid, (PFNTHREAD) PipeThread, &Pipes[i], 0,	/* immediate */
			    16384);
#endif
	    Pipes[i].used = 1;
