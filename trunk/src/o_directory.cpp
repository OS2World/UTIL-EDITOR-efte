...
EDirectory::EDirectory(int createFlags, EModel ** ARoot,
		       char *aPath):EList(createFlags, ARoot, aPath)
{
    char XPath[MAXPATH];
...
void EDirectory::DrawLine(PCell B, int Line, int Col, ChColor color,
			  int Width)
{
    char s[1024];
...
{
    return (Line >= 0
	    && Line < FCount) ? Files[Line]->Type() == fiDIRECTORY : 0;
}
...
void EDirectory::RescanList()
{
    ...
    Count = 0;
    FCount = 0;
    if (JustDirectory(Path, Dir, sizeof(Dir)) != 0)
	return;
	...
	sprintf(CTitle, "%d files%c%d dirs%c%lu bytes%c%-200.200s",
		...
		SizeCount, ConGetDrawChar(DCH_V), Dir);
	SetTitle(CTitle);
    }
...
int EDirectory::FmMvFile(const char *Name)
{
    ...
    strcpy(Dir, Path);
    if (View->MView->Win->
	GetStr("New file name", sizeof(Dir), Dir, HIST_PATH) == 0) {
	return 0;
    }
...
int EDirectory::FmRmFile(char const *Name)
{
    ...
    int choice = View->MView->Win->Choice(GPC_CONFIRM,
					  "Remove File",
					  2, "O&K", "&Cancel",
					  "Remove %s?", Name);
...
int EDirectory::FmMkDir()
{
    ...
    strcpy(Dir, Path);
    if (View->MView->Win->
	GetStr("New directory name", sizeof(Dir), Dir, HIST_PATH) == 0) {
	return 0;
    }
...
int EDirectory::ChangeDir(ExState & State)
{
	...
	strcpy(Dir, Path);
	if (View->MView->Win->
	    GetStr("Set directory", sizeof(Dir), Dir, HIST_PATH) == 0)
	    return 0;
    }
    if (ExpandPath(Dir, Dir2, sizeof(Dir2)) == -1)
	return 0;
