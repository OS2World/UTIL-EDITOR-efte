...
int Hilit_SIMPLE(EBuffer * BF, int LN, PCell B, int Pos, int Width,
		 ELine * Line, hlState & State, hsState * StateMap, int *ECol)
{
	...
	if (quotech) {
		...
		if ((matchFlags & (MATCH_SET | MATCH_NOTSET)) == 0) {
			...
			if (!RxExecMatch
			    (tr->regexp, Line->Chars, Line->Count, p, &b,
			     (matchFlags & MATCH_NO_CASE) ? 0 : RX_CASE))
			    continue;
			if (b.Open[1] != -1 && b.Close[1] != -1)
			    matchLen = b.Open[1] - i;
			else
			    matchLen = b.Close[0] - i;
		    }
		...
		while (((i + j) < Line->Count) &&
		       (WGETBIT(st->wordChars, Line->Chars[i + j])))
		    j++;
