...
int RxMatch(RxNode * rx)
{
	...
	case RE_ATBOW:
		...
		return 0;
	    if (rex > bop) {
		if ((ChClass(*rex) != 1) || (ChClass(*(rex - 1)) != 0))
		    return 0;
	    }
	    break;
	case RE_ATEOW:
		...
		return 0;
	    if (rex < eop) {
		if ((ChClass(*rex) != 0) || (ChClass(*(rex - 1)) != 1))
		    return 0;
	    }
	    break;
	case RE_CHAR:
	...
	case RE_BREAK:
		...
		while ((n->fWhat & RE_BRANCH) && n->fPtr
		       && ((n->fWhat & 0xFF) == '|'))
		    n = n->fPtr->fNext;
	    }
	    if (n->fWhat & RE_GROUP) {
...
int RxExecMatch(RxNode * Regexp, const char *Data, int Len, const char *Start,
		RxMatchRes * Match, unsigned int RxOpt)
{
    if (Regexp == 0)
	return 0;
...
int RxExec(RxNode * Regexp, const char *Data, int Len, const char *Start,
	   RxMatchRes * Match, unsigned int RxOpt)
{
    char Ch;
...
int RxReplace(const char *rep, const char *Src, int /*len */ ,
	      RxMatchRes match, char **Dest, int *Dlen)
{
    int dlen = 0;
    char *dest = 0;
    char Ch;
    ...
    *Dlen = 0;
	...
	case '\\':
		...
		if (match.Open[n] != -1 && match.Close[n] != -1) {
		    add(&dlen, &dest, Src + match.Open[n],
			match.Close[n] - match.Open[n], flag);
		}
		else
		    return -1;
		break;
