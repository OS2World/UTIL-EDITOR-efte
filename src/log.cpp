...
bool GlobalLog::OpenLogFile()
{
	...
	m_ofsLog.open(m_strLogFile,
		      ios::out /*| ios::text */  | ios::app /*append */ );
	if (!m_ofsLog) {
	    m_strLogFile = NULL;
	    m_bOpened = false;
	}
...
ostream & GlobalLog::operator()() {
	...
	<< setw(2) << ptm->tm_hour << ':'
	<< setw(2) << ptm->tm_min << ':'
	<< setw(2) << ptm->tm_sec << ' ' << "eFTE" << ' ';
    m_ofsLog.fill(cOldFill);
    return m_ofsLog;
}

FunctionLog::FunctionLog(GlobalLog & gl, const char *funcName,
			 unsigned long line)
...
ostream & Log__osBinChar(ostream & os, char const &c)
{
    ...
    os << (isprint(c) ? c : '.') << " [0x" << hex << (int)c << dec << "]";
    os.fill(cOldFill);
    return os;
}
