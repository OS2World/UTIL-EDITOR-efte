...
BufferView::BufferView(int createFlags, EModel ** ARoot):EList(createFlags,
							       ARoot,
							       "Buffers")
{
    ModelNo = 0;			// trick
...
void BufferView::DrawLine(PCell B, int Line, int Col, ChColor color,
			  int Width)
{
    if (Line < BCount)
...
int BufferView::getMatchingLine(int start, int direction)
{
	...
	for (int j = 0; BList[i][j]; j++)
	    if (BList[i][j] == SearchString[0]
		&& strnicmp(SearchString, BList[i] + j, SearchLen) == 0) {
		return i;
	    }
	i += direction;
	if (i == BCount)
...
void BufferView::GetTitle(char *ATitle, int MaxLen, char *ASTitle,
			  int SMaxLen)
{
