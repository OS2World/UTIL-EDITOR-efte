...
void ExISearch::HandleEvent(TEvent & Event)
{
	...
	case kbBackSp:
		...
		if (stacklen > 0) {
		    stacklen--;
		    if (Buffer->
			CenterPos(stack[stacklen].Col,
				  stack[stacklen].Row) == 0)
			return;
		}
		len--;
		ISearchStr[len] = 0;
		if (len > 0
		    && Buffer->FindStr(ISearchStr, len,
				       Case | Direction) == 0) {
		    SetState(INoMatch);
		}
	    }
	    else {
		if (Buffer->CenterPos(Orig.Col, Orig.Row) == 0)
		    return;
	    }
	    break;
	case kbUp:
	...
	case kbTab | kfShift:
	    ...
	    if (Buffer->
		FindStr(ISearchStr, len,
			Case | Direction | SEARCH_NEXT) == 0) {
