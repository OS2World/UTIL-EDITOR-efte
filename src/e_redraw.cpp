...
int EBuffer::GetMap(int Row, int *StateLen, hsState ** StateMap)
{
	...
	*StateMap = (hsState *) malloc(*StateLen);
	...
	if (BFI(this, BFI_HilitOn) == 1 && HilitProc != 0)
	    ...
	    Hilit_Plain(this, Row, 0, 0, *StateLen, L, State, *StateMap,
			&ECol);
	//        if (L->StateE != State) {
	//            L->StateE = State;
	//        }
...
void EBuffer::DrawLine(TDrawBuffer B, int VRow, int C, int W, int &HilitX)
{
	...
	if (Row < RCount) {
		...
		if (FF[f].level < 5)
		    foldColor = hcPlain_Folds[FF[f].level];
		else
...
void EBuffer::Redraw()
{
	...
	if (V == View) {
	    ...
	    if (W->TP.Row + scrollBorderY > CP.Row)
		W->TP.Row = CP.Row - scrollJumpY + 1 - scrollBorderY;
	    if (W->TP.Row + W->Rows - scrollBorderY <= CP.Row)
		W->TP.Row =
		    CP.Row - W->Rows + 1 + scrollJumpY - 1 + scrollBorderY;
	    if (!WeirdScroll)
		if (W->TP.Row + W->Rows >= VCount)
		    W->TP.Row = VCount - W->Rows;
	    if (W->TP.Row < 0)
		W->TP.Row = 0;

	    if (W->TP.Col + scrollBorderX > CP.Col)
		W->TP.Col = CP.Col - scrollJumpX - scrollBorderX;
	    if (W->TP.Col + W->Cols - scrollBorderX <= CP.Col)
		W->TP.Col = CP.Col - W->Cols + scrollJumpX + scrollBorderX;
	    if (W->TP.Col < 0)
		W->TP.Col = 0;

	    if (W->OldTP.Row != -1 && W->OldTP.Col != -1 && RedrawToEos == 0) {

		if ((W->OldTP.Row != W->TP.Row)
		    || (W->OldTP.Col != W->TP.Col)) {
		    int A, B;
		    int DeltaX, DeltaY;
		    int Rows = W->Rows;
	...
	if (V->MView->Win->GetViewContext() == V->MView) {
	    V->MView->Win->SetSbVPos(W->TP.Row, W->Rows,
				     VCount + (WeirdScroll ? W->Rows -
					       1 : 0));
	    V->MView->Win->SetSbHPos(W->TP.Col, W->Cols,
				     1024 + (WeirdScroll ? W->Cols - 1 : 0));
	}
	...
	if (V->CurMsg == 0) {
		...
		sprintf(s, "%04d:%02d (%04d:%02d) %c%c%c%c%c %.6s %c",
			//                    CurLine + 1,
			ActLine + 1, CurColumn + 1, NumLines, NumChars,
			//                    CurPos + 1,
	...
	if (V->MView->Win->GetStatusContext() == V->MView) {
	    ...
	    if (V->MView->IsActive()) {
		V->MView->ConSetCursorPos(W->CP.Col - W->TP.Col,
					  W->CP.Row - W->TP.Row);
	...
	for (int R = W->TP.Row; R < W->TP.Row + W->Rows; R++) {
	    Row = R;
	    if ((Row >= RowA) && (RedrawToEos || Row <= RowZ)) {
