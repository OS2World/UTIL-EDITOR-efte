...
static int DrawHMenu(int x, int y, int id, int active)
{
	...
	for (i = 0; i < Menus[id].Count; i++) {
		...
		MoveCStr(B, pos + 1, Cols, Menus[id].Items[i].Name, color1,
			 color2, len);
		pos += len + 2;
	    }
	    else {
...
{
    if (Y <= 0 || Y > Menus[id].Count)
	return -1;
    if (Menus[id].Items[Y - 1].Name == 0)
	return -1;
    if (X <= 0 || X >= w - 1)
	return -1;
    return Y - 1;
...
int ExecVertMenu(int x, int y, int id, TEvent & E, UpMenu * up)
{
	...
	ConHideCursor();
	do {
	    ConGetEvent(evCommand | evMouseDown | evMouseMove | evMouseUp |
			evKeyDown | evNotify, &E, -1, 1);
	    if (E.What & evNotify)
		...
		if (isAscii(E.Key.Code)) {
			...
			if (Menus[id].Items[i].Name) {
				...
				if (toupper(o[1]) == cc) {
				    cur = i;
				    if (cur != -1) {
					if (Menus[id].Items[cur].SubMenu ==
					    -1) {
					    E.What = evCommand;
					    E.Msg.View = frames->Active;
					    E.Msg.Command =
						Menus[id].Items[cur].Cmd;
					    abort = 1;
					}
					else {
					    dovert = 1;
		...
		if (E.Mouse.X >= x && E.Mouse.Y >= y &&
			...
			while (p) {
				...
				if (first == 1) {
				    if (p->vert) {
					int i =
					    GetVPosItem(p->id, p->w,
							E.Mouse.X - p->x,
							E.Mouse.Y - p->y);
					if (i != -1)
					    if (Menus[p->id].Items[i].
						SubMenu == id)
						break;
				    }
				    else {
	...
	case evMouseUp:
		...
		if (up) {
			...
			if (p->vert) {
			    int i =
				GetVPosItem(p->id, p->w, E.Mouse.X - p->x,
					    E.Mouse.Y - p->y);
			    if (i != -1)
				if (Menus[p->id].Items[i].SubMenu == id)
				    break;
			}
			else {
			    int i = GetHPosItem(p->id, E.Mouse.X);
