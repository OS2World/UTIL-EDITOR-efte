...
int FileLoad(int createFlags, const char *FileName, const char *Mode,
	     EView * View)
{
    char Name[MAXPATH];
    EBuffer *B;
...
int MultiFileLoad(int createFlags, const char *FileName, const char *Mode,
		  EView * View)
{
    char fX[MAXPATH];
    int count = 0;
    ...
    if (ExpandPath(fX, FPath, sizeof(FPath)) == -1)
	return 0;
    Slash(FPath, 1);
