...
int EBuffer::CharTrans(TransTable tab)
{
    ...
    if (P >= (unsigned int)L->Count)
	return 0;
    if (ChgChars(CP.Row, P, 1, 0) == 0)
	return 0;
...
int EBuffer::LineTrans(TransTable tab)
{
    ...
    if (L->Count > 0) {
	if (ChgChars(CP.Row, 0, L->Count, 0) == 0)
	    return 0;
	for (I = 0; I < L->Count; I++)
...
int EBuffer::GetTrans(ExState & State, TransTable tab)
{
	...
	if (View->MView->Win->
	    GetStr("Trans From", sizeof(TrS), (char *)TrS, HIST_TRANS) == 0)
	    return 0;
    if (State.GetStrParam(View, (char *)TrD, sizeof(TrD)) == 0)
	if (View->MView->Win->
	    GetStr("Trans To", sizeof(TrS), (char *)TrD, HIST_TRANS) == 0)
	    return 0;
