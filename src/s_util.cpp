...
static int copyfile(char *f1, char *f2)
{					// from F1 to F2
    void *buffer;
    int fd1, fd2;
    int rd;
...
char *MakeBackup(char *FileName, char *NewName)
{
	...
	for (size_t idx = 0; idx < strlen(TmpFileName); idx++)
		...
		|| TmpFileName[idx] == ':')
		TmpFileName[idx] = '_';
	snprintf(TmpBackupName, MAXPATH, "%s/%s", BackupDirectory,
		 TmpFileName);
...
int CompletePath(const char *Base, char *Match, int Count)
{
    ...
    if (strcmp(Base, "") == 0) {
	if (ExpandPath(".", Name, sizeof(Name)) != 0)
	    return -1;
    }
    else {
	if (ExpandPath(Base, Name, sizeof(Name)) != 0)
	    return -1;
    }
//    SlashDir(Name);
    dirp = Name;
	...
	*namep = 0;
    ...
    ff = new FileFind(dirp, "*", ffDIRECTORY | ffHIDDEN
	...
	if ((strcmp(dname, ".") != 0) &&
	    (strcmp(dname, "..") != 0) &&
	    (!CompletionFilter
	     || RxExec(CompletionFilter, dname, strlen(dname), dname,
		       &RM) != 1)) {
	    if ((
#if defined(UNIX)
		    strncmp
...
#ifdef UNIX
			while (cname[o] && dname[o] && (cname[o] == dname[o]))
			    o++;
#endif
#if defined(OS2) || defined(NT)
			while (cname[o] && dname[o]
			       && (toupper(cname[o]) == toupper(dname[o])))
			    o++;
#endif
			cname[o] = 0;
		    }
