...
static void DrawCursor(int Show)
{
	...
	((cci.dwSize == 100 && CursorInsert)
	 || (cci.dwSize == 10 && CursorInsert == 0)))
	return;
...
TransCharScan[] =
{
    ...
    0x1C00, kbEnter}
    , {
    0xE00D, kbEnter | kfGray}
    , {
    0xA600, kbEnter | kfGray}
    , {
    0xE00A, kbEnter | kfGray}
    , {
    0x0E08, kbBackSp}
    , {
    0x0E7F, kbBackSp}
...
int ReadConsoleEvent(TEvent * E)
{
    ...
    if (nread != 1)
	return False;			// Nothing read after signal??

    switch (inp.EventType) {
    case WINDOW_BUFFER_SIZE_EVENT:
	//** Resized the window. Make FTE use the new size..
	frames->Resize(inp.Event.WindowBufferSizeEvent.dwSize.X,
		       inp.Event.WindowBufferSizeEvent.dwSize.Y);
	frames->Repaint();
	return True;
	...
	if (flags & (RIGHT_CTRL_PRESSED | LEFT_CTRL_PRESSED))
	    flg |= kfCtrl;
	if (flags & (RIGHT_ALT_PRESSED))
	    flg &= ~kfCtrl;
	if (flags & SHIFT_PRESSED)
	    flg |= kfShift;
	if (flags & ENHANCED_KEY)
	    flg |= kfGray;
	...
	if (Ch == 0) {
	    unsigned int cc =
		((inp.Event.KeyEvent.
		  wVirtualScanCode << 8) | (unsigned char)inp.Event.KeyEvent.
		 uChar.AsciiChar);
	...
	if (Ch == 0) {
	    if ((Ch =
		 (TKeyCode) (unsigned char)inp.Event.KeyEvent.uChar.
		 AsciiChar) != 0) {
		if (flg & kfAlt)
		    Ch = toupper(Ch);
	    }
	...
	if (inp.Event.MouseEvent.dwEventFlags & MOUSE_WHEELED) {
		...
		if (flg & kfShift) {	// Translate to horizontal scroll.
		    E->Msg.Command =
			(flg & kfCtrl) ? cmHScrollPgRt : cmHScrollRight;
		}
		else {			// Translate to vertical scroll.
		    E->Msg.Command =
			(flg & kfCtrl) ? cmVScrollPgDn : cmVScrollDown;
		}
	    }
	    else {			// Wheel up
		if (flg & kfShift) {	// Translate to horizontal scroll.
		    E->Msg.Command =
			(flg & kfCtrl) ? cmHScrollPgLt : cmHScrollLeft;
		}
		else {			// Translate to vertical scroll.
		    E->Msg.Command =
			(flg & kfCtrl) ? cmVScrollPgUp : cmVScrollUp;
		}
	    }
	...
	if (inp.Event.MouseEvent.dwEventFlags == MOUSE_MOVED) {
		...
		E->What = evMouseDown;
		E->Mouse.Buttons =
		    ((unsigned short)inp.Event.MouseEvent.
		     dwButtonState) & ~mb;
		//puts("Down");
	    }
	    else {
		E->What = evMouseUp;
		E->Mouse.Buttons =
		    mb & ~((unsigned short)inp.Event.MouseEvent.
			   dwButtonState);
		//puts("Up");
	    }
...
int ConClear(void)
{
    ...
    if ((ConQuerySize(&W, &H) == 0) && ConSetBox(0, 0, W, H, B[0]))
	return 0;
    return -1;
}
...
int ConPutLine(int X, int Y, int W, int H, PCell Cell)
{
	...
	rc = WriteConsoleOutput(OurConOut, (PCHAR_INFO) Cell, csize, corg,
				&rcl);
	if (rc != TRUE) {
...
int ConScroll(int Way, int X, int Y, int W, int H, TAttr Fill, int Count)
{
    ...
    ScrollConsoleScreenBuffer(OurConOut, &rect, &clip, dest,
			      (PCHAR_INFO) & FillCell);
    return 0;
}
...
int ConSetTitle(char *Title, char *STitle)
{
    ...
    strlcpy(winTitle, "eFTE - ", sizeof(winTitle));
...
int GPipe::createPipe()
{
    ...
    Pipes[i].tid = _beginthread(PipeThread,
				FAKE_BEGINTHREAD_NULL 16384, &Pipes[i]);
#else

    //** Create the named pipe, and handle the SERVER (edit)'s end...
    sprintf(pipename, "\\\\.\\pipe\\efte%d\\child%d", getpid(), PCount);
    p_pipe_ph = CreateNamedPipe(pipename, PIPE_ACCESS_INBOUND | FILE_FLAG_OVERLAPPED, PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT, 1,	// nMaxInstances,
				0, PIPEBUF_SZ, 1000, 0);
...
void GPipe::closeProc()
{
    ...
    if (!GetExitCodeProcess(p_proc_h, &ec))
	ec = 0xabcd;
...
int GPipe::getEvent(TEvent * event)
{
    ...
    if (!p_used || p_notify == 0)
	return 0;			// No data.
    if (!handlePost())
	return 0;			// Again: no data,
    //** This pipe has data!
    event->What = evNotify;
    event->Msg.View = 0;
...
#endif
	    if (CreateProcess
		(NULL, args, NULL, NULL, TRUE, 0, NULL, NULL, &si,
		 &pi) == TRUE) {
		ok = TRUE;
		CloseHandle(pi.hThread);	// Thread handle not needed
		...
		int Delete)
{
	...
	*Event = MouseEv;
	...
	if (rc != WAIT_FAILED
	    && (rc >= WAIT_OBJECT_0 && rc < WAIT_OBJECT_0 + nh)) {
