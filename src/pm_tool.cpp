...
MRESULT EXPENTRY ToolBarProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	...
	case WM_PAINT:{
		...
		if (rcl.yTop >= swp.cy - TYBORDER + 2) {
		    for (ptl.y = swp.cy - TYBORDER + 2; ptl.y < swp.cy - 1;
			 ptl.y++) {
			ptl.x = Max(rcl.xLeft, 1);
			GpiMove(hps, &ptl);
			ptl.x = Min(rcl.xRight, swp.cx - 1);
		...
		for (item = 0; item < td->ulCount; item++) {
			...
			if (rcl.xRight >= xpos - 2
			    && rcl.xLeft <= xpos + TXICON + 1) {
				...
				&& (items[item].ulFlags & tfDEPRESSED)) {
				ptl.x = xpos + 1;
				ptl.y = ypos - 1;
				WinDrawBitmap(hps,
					      items[item].hBitmap,
			...
			if (rcl.xRight >= xpos - 1
			    && rcl.xLeft <= xpos + TXSEPARATOR + 1) {
			    GpiSetColor(hps, CLR_PALEGRAY);
			    ptl.x = xpos - 1;
			    ptl.y = ypos - 2;
			    GpiMove(hps, &ptl);
		...
		for (item = 0; item < td->ulCount; item++) {
			...
			td->pItems[item].ulType == tiBITMAP &&
				...
				WinMapWindowPoints(hwnd, HWND_DESKTOP,
						   &pointl, 1);
				WinSetWindowPos(hwndBubble, HWND_TOP, pointl.x, pointl.y - 24, 110, 22,	// size of bubble
						SWP_SIZE | SWP_MOVE |
						SWP_SHOW);
				WinSetPresParam(hwndBubbleClient,
						PP_FONTNAMESIZE, 7, "8.Helv");
				RGB rgb = { 200, 200, 0 };
				...
				WinLoadString(hab, 0, td->pItems[item].ulId,
	...
	case WM_TIMER:
		...
		WinMapWindowPoints(HWND_DESKTOP,
				   WinQueryWindow(hwnd, QW_PARENT), &ptl, 1);
...
void RegisterToolBarClass(HAB hab)
{
    assert(WinRegisterClass(hab,
			    WC_MTOOLBAR,
			    (PFNWP) ToolBarProc,
			    CS_SIZEREDRAW, sizeof(void *)) == TRUE);
}

HWND CreateToolBar(HWND parent,
		   HWND owner, ULONG id, ULONG count, ToolBarItem * items)
{
    ...
    hwnd = WinCreateWindow(parent,
			   ...
			   "ToolBar",
			   WS_VISIBLE,
			   0, 0, 0, 0, owner, HWND_TOP, id, td, 0);
