...
static int AllocMem(char *Mem, int Len)
{					/*FOLD00 */
    int N = 1024;
    char *NM;
    int TagPos = TagLen;
...
{					/*FOLD00 */
	...
	if (GotoFilePos(View, path, 0, 0) == 0)
	    return 0;
	if (((EBuffer *) ActiveModel)->
	    FindStr(TagMem + TT->StrFind, strlen(TagMem + TT->StrFind),
		    0) == 0)
	    return 0;
    }
    ((EBuffer *) ActiveModel)->FindStr(TagMem + TT->Tag,
				       strlen(TagMem + TT->Tag), 0);
    return 1;
}
...
int TagFind(EBuffer * B, EView * View, char *Tag)
{					/*FOLD00 */
...
{					/*FOLD00 */
    ...
    if (TagPosition < CTags - 1
	&& strcmp(CurrentTag,
		  TagMem + TagD[TagI[TagPosition + 1]].Tag) == 0) {
	TagPosition++;
