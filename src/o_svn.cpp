...
static int SameDir(char *D1, char *D2)
{
    ...
    if (l1 < l2)
	return strncmp(D1, D2, l1) == 0 && strcmp(D2 + l1, SSLASH) == 0;
    else if (l1 == l2)
	return !strcmp(D1, D2);
    else
...
ESvn::ESvn(int createFlags, EModel ** ARoot, char *ADir, char *ACommand,
	   char *AOnFiles):ESvnBase(createFlags, ARoot, "SVN")
{
    SvnView = this;
    LogFile = 0;
    Commiting = 0;
...
ESvn::ESvn(int createFlags, EModel ** ARoot):ESvnBase(createFlags, ARoot,
						      "SVN")
{
    SvnView = this;
    LogFile = 0;
...
char *ESvn::MarkedAsList()
{
	...
	if (Lines[i]->Status & 2)
	    len += strlen(Lines[i]->File) + 1;
    if (len == 0) {
	// Nothing marked, use the file at cursor
	if (Lines[Row]->Status & 4)
	    return strdup(Lines[Row]->File);
	else
	    return NULL;
    }
...
char ESvn::GetFileStatus(char *file)
{
	...
	if (Lines[i]->File && filecmp(Lines[i]->File, file) == 0)
	    return Lines[i]->Msg[0];
    return 0;
}
...
int ESvn::DoneCommit(int commit)
{
	...
	char *ACommand =
	    (char *)malloc(strlen(Command) + strlen(LogFile) + 10);
...
int ESvn::ConfQuit(GxView * V, int multiFile)
{
	...
	switch (V->
		Choice(GPC_ERROR, "SVN command is running", 2, "&Kill",
		       "&Cancel", "")) {
	case 0:			// Kill
	    return 1;
	case 1:			// Cancel
