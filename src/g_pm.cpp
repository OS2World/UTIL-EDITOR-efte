...
typedef struct _PMPTR
{				// for passing pointers to winprocs
    USHORT len;
    void *p;
}
PMPTR;
...
unsigned long HaveGUIDialogs =
    GUIDLG_FILE |
    GUIDLG_CHOICE | GUIDLG_FIND | GUIDLG_FINDREPLACE | GUIDLG_PROMPT;
...
HMQ hmqW = 0;
HMTX hmtxPMData = 0;
ULONG cxScreen, cyScreen, cyTitleBar,	//cyMenuBar,
    cxBorder, cyBorder, cxScrollBar, cyScrollBar;
SHORT reportSize = 1;
...
HWND CreateToolBar(HWND parent, HWND owner, int id)
{
	...
	if (tools[i].handle == 0 && (tools[i].flags & MIS_BITMAP)) {
	    tools[i].handle =
		GpiLoadBitmap(ps, NULLHANDLE, tools[i].id, 0, 0);
	}
...
HWND CreateToolBar(HWND parent, HWND owner, int id)
{
	...
	if (tools[i].hBitmap == 0 && (tools[i].ulType == tiBITMAP))
	    tools[i].hBitmap =
		GpiLoadBitmap(hps, NULLHANDLE, tools[i].ulId, 0, 0);
    }

    WinReleasePS(hps);

    return CreateToolBar(parent, owner, id,
			 sizeof(tools) / sizeof(tools[0]), tools);
}
...
HWND CreatePMMenu(HWND parent, HWND owner, int menu, int id, int style)
{
	...
	if (Menus[menu].Items[i].Name) {
		...
		item.hwndSubMenu = CreatePMMenu(HWND_DESKTOP, owner,
						...
						0,
						(Menus[menu].Items[i].Cmd ==
						 SUBMENU_CONDITIONAL) ?
						MS_CONDITIONALCASCADE : 0);
		{
		    static int ids = 1000;
...
MRESULT EXPENTRY FileDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	...
	InsertHistory(WinWindowFromID(hwnd, DID_FILENAME_ED), HIST_PATH,
		      MAXPATH);
	WinInvalidateRect(hwnd, 0, TRUE);
	WinRestoreWindowPos("eFTEPM",
			    ((dlg->
			      fl & FDS_SAVEAS_DIALOG) ? "FileSaveDlg" :
			     "FileOpenDlg"), hwnd);
	break;
    case WM_COMMAND:
	switch (SHORT1FROMMP(mp1)) {
...
int DLGGetFile(GView * View, const char *Prompt, unsigned int BufLen,
	       char *FileName, int Flags)
{
    ...
    dlg.fl =
	FDS_CENTER | ((Flags & GF_SAVEAS) ? FDS_SAVEAS_DIALOG :
		      FDS_OPEN_DIALOG);
...
static int DoChoice(HWND hwndFrame, ChoiceInfo * choice)
{
	...
	WinDlgBox(HWND_DESKTOP, hwndFrame, AboutDlgProc, NULLHANDLE,
		  IDD_ABOUT, NULL);
	return 0;
    }
	...
	if (choice->Flags == GPC_NOTE)
	    dlgtype = IDD_INFO;
	else
	    dlgtype = IDD_ERROR;

	WinDlgBox(HWND_DESKTOP, hwndFrame, InfoDlgProc, NULLHANDLE, dlgtype,
		  msg);
	...
	hwndButton[i] =
	    WinCreateWindow(hwndDlg,
			    WC_BUTTON,
			    button,
			    WS_VISIBLE | BS_PUSHBUTTON | BS_AUTOSIZE |
			    ((i ==
			      0) ? BS_DEFAULT | WS_TABSTOP | WS_GROUP : 0),
			    cxBorder + x, SPC + cyBorder, 0, 0, hwndDlg,
			    ((i == 0) ? HWND_TOP : hwndButton[i - 1]),
			    200 + i, NULL, NULL);
    ...
    ny = cyBorder + SPC + swp.cy + SPC + swp1.cy + SPC + cyTitleBar +
	cyBorder;
    ...
    if (y < cyBorder)
	y = -cyBorder;
    if (y + ny >= cyScreen + cyBorder)
	y = cyScreen - ny + cyBorder;
    if (x + nx >= cxScreen + cxBorder)
	x = cxScreen - nx + cxBorder;
    if (x < -cxBorder)
	x = -cxBorder;

    WinSetWindowPos(hwndDlg, 0,
		    x, y,
		    nx, ny, SWP_SIZE | SWP_SHOW | SWP_MOVE | SWP_ACTIVATE);
...
int DLGPickChoice(GView * View, const char *ATitle, int NSel, va_list ap,
		  int Flags)
{
    int i;
    int rc;
    ...
    choice.NSel = NSel;
    for (i = 0; i < NSel; i++)
	choice.Buttons[i] = va_arg(ap, char *);
    choice.Format = va_arg(ap, char *);
    choice.Message = va_arg(ap, char *);

    choice.Flags = Flags;

    rc = LONGFROMMR(WinSendMsg
		    (View->Parent->Peer->hwndFrame, UWM_CHOICE,
		     MPFROMP(&choice), 0));
...
MRESULT EXPENTRY PromptDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	...
	WinSendDlgItemMsg(hwnd, IDE_FIELD, EM_SETTEXTLIMIT,
			  MPFROMLONG(PromptInfo.MaxLen), 0);
	WinSetDlgItemText(hwnd, IDE_FIELD, PromptInfo.Entry);
	InsertHistory(WinWindowFromID(hwnd, IDE_FIELD), PromptInfo.HistId,
		      PromptInfo.MaxLen);
	...
	switch (SHORT1FROMMP(mp1)) {
	case DID_OK:
	    WinQueryDlgItemText(hwnd, IDE_FIELD, PromptInfo.MaxLen,
				PromptInfo.Entry);
...
int DLGGetStr(GView * View, const char *Prompt, unsigned int BufLen,
	      char *Str, int HistId, int Flags)
{
    assert(BufLen > 0);
    ...
    if (LONGFROMMR(WinSendMsg(View->Parent->Peer->hwndFrame,
			      UWM_DLGBOX, MPFROMP(PFNWP(PromptDlgProc)),
			      MPFROMLONG(IDD_PROMPT))) != DID_OK)
	return 0;
...
MRESULT EXPENTRY FindDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	...
	if (ReplaceDlg) {
	    ...
	    WinCheckButton(hwnd, IDC_DELETELINE,
			   (SearchOpt.Options & SEARCH_DELETE) ? 1 : 0);
	}
	WinInvalidateRect(hwnd, 0, TRUE);
	WinRestoreWindowPos("eFTEPM", ReplaceDlg ? "ReplaceDlg" : "FindDlg",
			    hwnd);
	return WinDefDlgProc(hwnd, msg, mp1, mp2);
...
MRESULT CreateChild(HWND parent, GViewPeer * peer, PMData * pmData)
{
    ...
    peer->hwndVscroll = WinCreateWindow(parent,
					WC_SCROLLBAR, "",
					WS_VISIBLE | SBS_VERT | SBS_AUTOTRACK,
					0, 0, 0, 0, peer->hwndView, HWND_TOP,
					0, (void *)&ptr, NULL);
    ...
    peer->hwndHscroll = WinCreateWindow(parent,
					WC_SCROLLBAR, "",
					WS_VISIBLE | SBS_HORZ | SBS_AUTOTRACK,
					0, 0, 0, 0, peer->hwndView, HWND_TOP,
					0, (void *)&ptr, NULL);
...
MRESULT EXPENTRY FrameWndProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	...
	case UWM_FILEDIALOG:
	    return
		MRFROMLONG(WinFileDlg
			   (HWND_DESKTOP, hwnd,
			    (FILEDLG *) PVOIDFROMMP(mp1)));

	case UWM_DLGBOX:
	    return MRFROMLONG(WinDlgBox(HWND_DESKTOP, hwnd,
					PFNWP(PVOIDFROMMP(mp1)), 0,
					LONGFROMMP(mp2), NULL));
	...
	case UWM_CHOICE:
	    return
		MRFROMLONG(DoChoice(hwnd, (ChoiceInfo *) PVOIDFROMMP(mp1)));

	case UWM_CREATECHILD:
	    //DosBeep(2500, 1000);
	    return CreateChild(hwnd, (GViewPeer *) PVOIDFROMMP(mp1),
			       (PMData *) PVOIDFROMMP(mp2));
	...
	case WM_TRANSLATEACCEL:
		...
		USHORT fl =
		    (USHORT) (SHORT1FROMMP((qmsg->mp1)) &
			      (KC_ALT | KC_SHIFT | KC_CTRL | KC_KEYUP));
	...
	case WM_MINMAXFRAME:{
		...
		if (pswp->fl & SWP_MAXIMIZE) {
		    ...
		    pswp->cy =
			cyScreen - cyTitleBar - swpMenu.cy - swpToolBar.cy;
		    ...
		    pswp->cy +=
			cyBorder * 2 + cyTitleBar + swpMenu.cy +
			swpToolBar.cy;
	...
	case WM_ADJUSTWINDOWPOS:{
		...
		if (pswp->fl & (SWP_SIZE | SWP_MOVE | SWP_MAXIMIZE)) {
		    ...
		    if (ShowToolBar) {
			HWND hwndToolBar =
			    WinWindowFromID(hwnd, FID_MTOOLBAR);
	...
	case WM_QUERYFRAMECTLCOUNT:{
		SHORT sCount =
		    SHORT1FROMMR(peer->oldFrameProc(hwnd, msg, mp1, mp2));
		GView *v;
...
MRESULT EXPENTRY AVIOWndProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
	...
	DosRequestMutexSem(hmtxPMData, SEM_INDEFINITE_WAIT);
	{
	    ...
	    WinDefAVioWindowProc(hwnd, (USHORT) msg, (ULONG) mp1,
				 (ULONG) mp2);

	    if (cxClient <= pmData->cxChar || cyClient <= pmData->cyChar
		|| reportSize == 0) {
		DosReleaseMutexSem(hmtxPMData);
		break;
	    }
	...
	DosRequestMutexSem(hmtxPMData, SEM_INDEFINITE_WAIT);
	{
		...
		VioShowBuf(0, (SHORT) (MAXXSIZE * Peer->wH * 2),
			   pmData->hvps);
    ...
    case DM_DROP:{
	    ...
	    DrgQueryStrName(pDragItem->hstrSourceName, 100,
...
MRESULT EXPENTRY CreatorWndProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
    ...
    case UWM_CREATEWORKER:{
		...
		WinCreateWindow(HWND_OBJECT,
				...
				0,
				0, 0, 0, 0,
				HWND_OBJECT, HWND_TOP, 0, (void *)&ptr, NULL);
    ...
    case UWM_CREATEFRAME:{
	    ...
	    peer->oldFrameProc =
		WinSubclassWindow(peer->hwndFrame, (PFNWP) FrameWndProc);

	    if (ShowToolBar) {
		peer->hwndToolBar =
		    CreateToolBar(peer->hwndFrame, peer->hwndFrame,
				  FID_MTOOLBAR);
    ...
    case UWM_CREATEMAINMENU:{
		...
		if (len == -1) {
		    AttrFound = 0;
		    len =
			WinQueryPresParam(hwnd, PP_FONTNAMESIZE,
					  PP_FONTHANDLE, &AttrFound,
					  sizeof(font), font, 0);
		}
		WinDestroyWindow(hwnd);
	    }
	    ...
	    hwndMenu =
		CreatePMMainMenu(peer->hwndFrame, peer->hwndFrame, Menu);

	    if (len > 0)
		WinSetPresParam(hwndMenu, AttrFound, len, font);
	    WinSendMsg(peer->hwndFrame, WM_UPDATEFRAME, MPFROMLONG(FCF_MENU),
		       0);
    ...
    case UWM_CREATEPOPUPMENU:{
	    ...
	    len =
		WinQueryPresParam(peer->menuBar, PP_FONTNAMESIZE,
				  PP_FONTHANDLE, &AttrFound, sizeof(font),
				  font, 0);
	    if (len > 0)
		WinSetPresParam(hwnd, AttrFound, len, font);

	    WinPopupMenu(HWND_DESKTOP, peer->Frame->Active->Peer->hwndView,
			 hwnd, ptl.x, ptl.y, 0,
			 PU_HCONSTRAIN | PU_VCONSTRAIN | PU_NONE | PU_KEYBOARD
...
int ConGetEvent(TEventMask EventMask, TEvent * Event, int WaitTime,
		int Delete, GView ** view)
{
    QMSG qmsg;
    GFrame *f;
    GView *v;
	...
	*Event = EventBuf;
	...
	while (frames && v) {
		...
		case WM_COMMAND:
			...
			Event->Msg.View = v->Parent->Active;
			Event->Msg.Command =
			    (SHORT1FROMMP(qmsg.mp1) - 8192) + 65536;
			frames = f;
			return 0;
		    }
		    break;
		...
		case WM_MOUSEMOVE:
		    ...
		    Event->Msg.View = v;
		    Event->Mouse.X =
			((SHORT) SHORT1FROMMP(qmsg.mp1)) /
			v->Peer->pmData->cxChar;
		    Event->Mouse.Y =
			v->Peer->wH -
			((SHORT) SHORT2FROMMP(qmsg.mp1)) /
			v->Peer->pmData->cyChar - 1;
...
static void _LNK_CONV WorkThread(void *)
{
    ...
    hwndCreatorWorker = WinCreateWindow(HWND_OBJECT,
					...
					0,
					0, 0, 0, 0,
					HWND_OBJECT, HWND_TOP, 0, NULL, NULL);
...
//    wY = 0;
    ...
    WinSendMsg(hwndCreatorWorker, UWM_CREATEWORKER, MPFROMP(this),
	       MPFROMP(pmData));
...
int GViewPeer::ConPutBox(int X, int Y, int W, int H, PCell Cell)
{
	...
	if (I + Y == cY)
	    Hidden = PMHideCursor();
	VioWrtCellStr(p, (USHORT) (W << 1), (USHORT) (Y + I), (USHORT) X,
		      pmData->hvps);
	if (Hidden)
	    PMShowCursor();
	p += W << 1;
...
int GViewPeer::ConGetBox(int X, int Y, int W, int H, PCell Cell)
{
    ...
    for (I = 0; I < H; I++) {
	VioReadCellStr((char *)p, &WW, (USHORT) (Y + I), (USHORT) X,
		       pmData->hvps);
	p += W << 1;
    }
    DosReleaseMutexSem(hmtxPMData);
...
int GViewPeer::ConSetBox(int X, int Y, int W, int H, TCell Cell)
{
	...
	if (I + Y == cY)
	    Hidden = PMHideCursor();
	VioWrtNCell(p, (USHORT) (W), (USHORT) (Y + I), (USHORT) X,
		    pmData->hvps);
	if (Hidden)
	    PMShowCursor();
    }
...
int GViewPeer::ConScroll(int Way, int X, int Y, int W, int H, TAttr Fill,
			 int Count)
{
    ...
    switch (Way) {
    case csUp:
	VioScrollUp((USHORT) Y, (USHORT) X, (USHORT) (Y + H - 1),
		    (USHORT) (X + W - 1), (USHORT) Count, (PBYTE) & FillCell,
		    pmData->hvps);
	break;
    case csDown:
	VioScrollDn((USHORT) Y, (USHORT) X, (USHORT) (Y + H - 1),
		    (USHORT) (X + W - 1), (USHORT) Count, (PBYTE) & FillCell,
		    pmData->hvps);
	break;
    case csLeft:
	VioScrollLf((USHORT) Y, (USHORT) X, (USHORT) (Y + H - 1),
		    (USHORT) (X + W - 1), (USHORT) Count, (PBYTE) & FillCell,
		    pmData->hvps);
	break;
    case csRight:
	VioScrollRt((USHORT) Y, (USHORT) X, (USHORT) (Y + H - 1),
		    (USHORT) (X + W - 1), (USHORT) Count, (PBYTE) & FillCell,
		    pmData->hvps);
	break;
    }
...
int GViewPeer::ExpandHeight(int DeltaY)
{
    ...
    View->Next->ConSetSize(View->Next->Peer->wW,
			   View->Next->Peer->wH - DeltaY);
...
int GViewPeer::SetSbVPos(int Start, int Amount, int Total)
{
	...
	if (sbVamount < sbVtotal) {
	    ...
	    WinSendMsg(hwndVscroll, SBM_SETTHUMBSIZE, MPFROM2SHORT(0, 0), 0);
	    WinSendMsg(hwndVscroll, SBM_SETSCROLLBAR,
...
int GViewPeer::PMSetCursorPos()
{
	...
	WinCreateCursor(hwndView,
			...
			pmData->cxChar, pmData->cyChar, CURSOR_TYPE, NULL);
	WinShowCursor(hwndView, TRUE);
    }
    return 1;
}
...
int GView::ConScroll(int Way, int X, int Y, int W, int H, TAttr Fill,
		     int Count)
{
...
GFramePeer::GFramePeer(GFrame * aFrame, int Width, int Height)
{
    ...
    WinSendMsg(hwndCreatorUser, UWM_CREATEFRAME, MPFROMP(this),
	       MPFROMLONG(0));
...
int GFramePeer::ConGetTitle(char *Title, int MaxLen, char *STitle,
			    int SMaxLen)
{
...
void GFramePeer::MapFrame()
{
	...
	|| WinRestoreWindowPos("eFTEPM", "Frame1", hwndFrame) == FALSE) {
	WinQueryTaskSizePos(hab, 0, &swp);

	WinSetWindowPos(hwndFrame, HWND_TOP,
			swp.x, swp.y, swp.cx, swp.cy, SWP_MOVE | SWP_SIZE);
    }
...
void GFramePeer::SizeFrame()
{
	...
	WinQueryWindowPos(hwndToolBar, &swpToolBar);
	WinSendMsg(hwndToolBar,
		   WM_ADJUSTWINDOWPOS, MPFROMP(&swpToolBar), MPARAM(0));
    }
    else {
	swpToolBar.cy = 0;
    }
    ...
    WinQueryWindowPos(hwndMenu, &swpMenu);
    WinSendMsg(hwndMenu, WM_ADJUSTWINDOWPOS, MPFROMP(&swpMenu), MPARAM(0));

    //WinEnableWindowUpdate(hwndFrame, FALSE);
    DosRequestMutexSem(hmtxPMData, SEM_INDEFINITE_WAIT);
    ptl.x =
	v->Peer->wW * v->Peer->pmData->cxChar + cxScrollBar + 2 * cxBorder;
    ...
    swp.cx = ptl.x;
    swp.cy = ptl.y;
    WinSendMsg(hwndFrame, WM_ADJUSTWINDOWPOS, MPFROMP(&swp), MPARAM(0));
...
void GFrame::RemoveView(GView * view)
{
	...
	if (Top == view) {
	    Top = view->Next;
	    Top->ConSetSize(Top->Peer->wW,
			    Top->Peer->wH + view->Peer->wH + 1);
	}
	else {
...
int GFrame::ExecMainMenu(char Sub)
{
	...
	for (SHORT i = 0; i < count; i++) {
	    id = SHORT1FROMMR(WinSendMsg
			      (hwnd, MM_ITEMIDFROMPOSITION, MPFROMSHORT(i),
			       0));
	    if (id == MIT_ERROR) {
		puts("error");
	    }
		...
		len =
		    SHORT1FROMMR(WinSendMsg
				 (hwnd, MM_QUERYITEMTEXT,
				  MPFROM2SHORT((id), (sizeof(buf) - 1)),
				  MPFROMP(buf)));
		buf[len] = 0;
		//puts(buf);
		srch[1] = (char)toupper(Sub);
		if (strstr(buf, srch) != 0) {
		    ...
		    if (strstr(buf, srch) != 0) {
			WinSendMsg(hwnd, MM_SELECTITEM, (MPFROM2SHORT(id, 0)),
				   0);
			//printf("select %d %d\n", i, id);
		    }
		}
	...
	WinPostMsg(hwnd, MM_STARTMENUMODE,
		   MPFROM2SHORT((Sub ? TRUE : FALSE), FALSE), 0);
    }
    return 0;
}
...
int GFrame::PopupMenu(const char *Name)
{
    ...
    WinSendMsg(hwndCreatorUser,
	       UWM_CREATEPOPUPMENU, MPFROMP(Peer), MPFROMLONG(id));
    return 0;
}
...
int GUI::ConGetEvent(TEventMask EventMask, TEvent * Event, int WaitTime,
		     int Delete, GView ** view)
{
...
void GUI::ProcessEvent()
{
    ...
    E.What = evNone;
    if (E.What == evNone) {
	if (ConGetEvent(evMouseDown | evCommand | evKeyDown, &E, 0, 1, &v) ==
	    -1) ;
    }
    if (E.What == evNone) {
	//DosBeep(500, 100);
	frames->Update();
	//DosBeep(1000, 100);
	if (ConGetEvent(evMouseDown | evCommand | evKeyDown, &E, -1, 1, &v) ==
	    -1) ;
    }
    if (E.What != evNone) {
	v = E.Msg.View;
...
static int CreatePipeChild(ULONG * sid, PID * pid, HPIPE & hfPipe,
			   char *Command)
{
    static int PCount = 0;
    char szPipe[32];
...
#if 0
    rc = DosExecPgm(FailBuf, sizeof(FailBuf), EXEC_ASYNCRESULT,	// | EXEC_BACKGROUND,
		    Args, 0, &rc_code, Prog);
#else
    {
	STARTDATA sd;
...
static void _LNK_CONV PipeThread(void *p)
{
	...
	pipe->reading = 0;
	if (pipe->notify)
	    WinPostMsg(frames->Active->Peer->hwndWorker, UWM_NOTIFY,
		       MPFROMLONG(pipe->notify), MPFROMLONG(pipe->id));
	UninstallExceptq(&exRegRec);
...
//    fprintf(stderr, "Pipe: pClose: %d\n", pipe->id);
    ...
    //fprintf(stderr, "Pipe: pClose: %d\n", pipe->id);
#if 0
    rc = DosWaitChild(DCWA_PROCESS, DCWW_WAIT, &rc_code, &pid, pid);
...
{
    static char tab[] =
	"\xDA\xBF\xC0\xD9\xC4\xB3\xC2\xC3\xB4\xC1\xC5\x1A\xFA\x04\xC4\x18\x19\xB1\xB0\x1B\x1A";
