...
static int ftesl_get_ctrlcode(TKeyCode key)
{
    ...
    if (kcode >= kbF1 && kcode <= kbF12)
	return FTESL_KBDCTRL('f');
    else
	return (int)kcode;
...
int main(int argc, char *argv[])
{
    ...
    printf
	("Linux keymap compiler for SLang eFTE, Copyright (c) 1998 by István Váradi\n\n");

    if (argc < 3) {
	fprintf(stderr, "Usage: compkeys infile outfile\n\n");
	fprintf(stderr, "    where:\n");
	fprintf(stderr,
		"      infile:     the file with the list of the keys\n");
	fprintf(stderr,
		"      outfile:    the name of the output keymap file\n");
	exit(-2);
    }
	...
	case 1:
	    ...
	    break;
	case 2:
	    fprintf(stderr, "invalid key specification: '%s'", keyspecbuf);
	}
	fprintf(stderr, "\n");
	remove(foutname);
    }
