...
int EGUI::ExecMacro(GxView * view, int Macro)
{
	...
	i = State.Pos;
	if (m->cmds[i].type != CT_COMMAND || m->cmds[i].u.num == ExNop)
	    continue;

	for (j = 0; j < m->cmds[i].repeat; j++) {
	    State.Pos = i + 1;
	    if (ExecCommand(view, m->cmds[i].u.num, State) == 0
		&& !m->cmds[i].ign) {
		return ErFAIL;
	    }
	}
...
int EGUI::DesktopSaveAs(ExState & State, GxView * view)
{
	...
	if (view->
	    GetFile("Save Desktop", sizeof(DesktopFileName), DesktopFileName,
		    HIST_PATH, GF_SAVEAS) == 0)
	    return 0;
...
int EGUI::DesktopLoad(ExState & State, GxView * view)
{
    if (State.GetStrParam(0, DesktopFileName, sizeof(DesktopFileName)) == 0)
	if (view->
	    GetFile("Load Desktop", sizeof(DesktopFileName), DesktopFileName,
		    HIST_PATH, GF_OPEN) == 0)
	    return 0;
...
int EGUI::CmdLoadFiles(int &argc, char **argv)
{
	...
	if (!QuoteAll && !QuoteNext && (argv[Arg][0] == '-')) {
		...
		if (strchr(argv[Arg], ',')) {
		    GotoLine =
			(2 ==
			 sscanf(argv[Arg] + 2, "%d,%d", &LineNum, &ColNum));
		}
		else {
	    ...
	    QuoteNext = 0;
	    if (ExpandPath(argv[Arg], Path, sizeof(Path)) == 0
		&& IsDirectory(Path)) {
		...
		if (MultiFileLoad(cfAppend, argv[Arg],
				  ModeOverride ? Mode : 0, ActiveView) == 0) {
		    suspendLoads = 0;
		    return 0;
		}
		suspendLoads = 0;
		...
		if (GotoLine) {
		    ...
		    if (GotoLine) {
			GotoLine = 0;
			((EBuffer *) ActiveModel)->SetNearPosR(ColNum - 1,
							       LineNum - 1);
		    }
		    else {
			int r, c;
			...
			if (RetrieveFPos
			    (((EBuffer *) ActiveModel)->FileName, r, c) == 1)
