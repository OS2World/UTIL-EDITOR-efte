...
//    assert(sizeof(SavedScreen[i]) == MaxSavedH);
		SavedScreen[i] =
		    (PCell) realloc(SavedScreen[i],
				    NewSavedW * sizeof(TCell));
	    }
	    MaxSavedW = NewSavedW;
	}
	...
	if (NewSavedH > MaxSavedH) {	/* Expand Maximum height if needed */
	    SavedScreen =
		(PCell *) realloc(SavedScreen, NewSavedH * sizeof(PCell));
...
static int ConInitColors()
{
	...
	for (int bg = 0; bg < 8; bg++) {
		...
		for (int fg = 0; fg < 8; fg++, c++) {
			...
			if (c != 0)
			    init_pair(pair, fte_curses_colors[fg],
				      fte_curses_colors[bg]);
			fte_curses_attr[c] =
			    (fgb ? A_BOLD : 0) | COLOR_PAIR(pair);
		    }
		    else {
			fte_curses_attr[c] = 0;
			if (fgb || bgb) {
		...
		TEvent * Event, int WaitTime, int Delete)
{
	...
	*Event = Prev;
    ...
    if ((rtn = WaitPipeEvent(Event, -1, sfd, 1)) != 0)
	return rtn;
