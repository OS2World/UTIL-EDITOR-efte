...
EMessages::EMessages(int createFlags, EModel ** ARoot, char *ADir,
		     char *ACommand):EList(createFlags, ARoot, "Messages")
{
    CompilerMsgs = this;
    ErrCount = 0;
    ErrList = 0;
...
void EMessages::GetErrors()
{
	...
	if (!didmatch) {
		...
		if (strnicmp(pin, t1, sizeof(t1) - 1) == 0) {	// Entering?
			...
			AddError(0, -1, 0,
				 "efte: mismatch in directory stack!?");
...
int EMessages::CompilePrevError(EView * V)
{
	...
	if (Row > 0) {
	    Row--;
	    if (ErrList[Row]->line == -1 || !ErrList[Row]->file)
		goto bad;
	    ShowError(V, Row);
	}
	else {
...
int EMessages::CanActivate(int Line)
{
    ...
    if (Line < ErrCount)
	if (ErrList[Line]->file || ErrList[Line]->line != -1)
	    ok = 1;
    return ok;
}
...
void EMessages::GetInfo(char *AInfo, int /*MaxLen */ )
{
    sprintf(AInfo,
	    ...
	    ModelNo, Row, Count, MatchCount, Command);
}
