...
int EBuffer::MoveWordLeftX(int start)
{
	...
	if (P > 0) {
	    while ((P > 0)
		   && (WGETBIT(Flags.WordChars, L->Chars[P - 1]) == wE))
		P--;
	    while ((P > 0)
		   && (WGETBIT(Flags.WordChars, L->Chars[P - 1]) == wS))
		P--;
	    C = ScreenPos(L, P);
...
int EBuffer::MoveWordRightX(int start)
{
    ...
    while ((P < L->Count) && (WGETBIT(Flags.WordChars, L->Chars[P]) == wS))
	P++;
    while ((P < L->Count) && (WGETBIT(Flags.WordChars, L->Chars[P]) == wE))
	P++;
    C = ScreenPos(L, P);
    return SetPos(C, CP.Row);
}
...
int EBuffer::MoveWordOrCapLeft()
{
	...
	if (P > 0) {
	    while ((P > 0)
		   && (WGETBIT(Flags.WordChars, L->Chars[P - 1]) == 0))
		P--;
	    while ((P > 0) && (WGETBIT(Flags.WordChars, L->Chars[P - 1]) == 1)
		   && (WGETBIT(Flags.CapitalChars, L->Chars[P - 1]) == 0))
		P--;
	    while ((P > 0)
		   && (WGETBIT(Flags.CapitalChars, L->Chars[P - 1]) == 1))
		P--;
	    C = ScreenPos(L, P);
	    return SetPos(C, CP.Row);
	}
...
int EBuffer::MoveWordOrCapRight()
{
    ...
    while ((P < L->Count) && (WGETBIT(Flags.CapitalChars, L->Chars[P]) == 1))
	P++;
    while ((P < L->Count) && (WGETBIT(Flags.WordChars, L->Chars[P]) == 1)
	   && (WGETBIT(Flags.CapitalChars, L->Chars[P]) == 0))
	P++;
    while ((P < L->Count) && (WGETBIT(Flags.WordChars, L->Chars[P]) == 0))
	P++;
    C = ScreenPos(L, P);
    return SetPos(C, CP.Row);
}
...
{
    SetNearPos(CP.Col, GetVPort()->TP.Row + GetVPort()->Rows - 1, tmLeft);
    return 1;
}
...
{
    if (View)
	if (GetVPort()->SetTop(GetVPort()->TP.Col, CP.Row) == 0)
	    return 0;
    return 1;
}
...
int EBuffer::MoveLineCenter()
{
	...
	if (Row < 0)
	    Row = 0;
	if (GetVPort()->SetTop(GetVPort()->TP.Col, Row) == 0)
	    return 0;
    }
    return 1;
}
...
{
    if (PrevPos.Col == -1 || PrevPos.Row == -1)
	return 0;
    if (SetPosR(PrevPos.Col, PrevPos.Row) == 0)
	return 0;
    return 1;
}
...
int EBuffer::MoveSavedPosCol()
{
    if (SavedPos.Col == -1)
	return 0;
    if (SetPos(SavedPos.Col, CP.Row) == 0)
	return 0;
    return 1;
}
...
int EBuffer::MoveSavedPosRow()
{
    if (SavedPos.Row == -1)
	return 0;
    if (SetPosR(CP.Col, SavedPos.Row) == 0)
	return 0;
    return 1;
}
...
{
    if (SavedPos.Col == -1 || SavedPos.Row == -1)
	return 0;
    if (SetPosR(SavedPos.Col, SavedPos.Row) == 0)
	return 0;
    return 1;
}
...
int EBuffer::ScrollLeft(int Cols)
{
    ...
    if (SetNearPos(CP.Col + Cols, CP.Row, tmLeft) == 0)
	return 0;
    if (GetVPort()->SetTop(C + Cols, GetVPort()->TP.Row) == 0)
	return 0;
    return 1;
}
...
int EBuffer::ScrollDown(int Lines)
{
    ...
    if (SetNearPos(CP.Col, CP.Row - Lines, tmLeft) == 0)
	return 0;
...
int EBuffer::KillWord()
{
	...
	while ((P < L->Count) && (ChClassK(L->Chars[P]) == Class))
	    P++;
	C = ScreenPos(L, P);
	if (DelText(Y, CP.Col, C - CP.Col) == 0)
	    return 0;
    }
    return 1;
}
...
int EBuffer::KillWordPrev()
{
	...
	while ((P > 0) && (ChClassK(L->Chars[P - 1]) == Class))
	    P--;
	C = ScreenPos(L, P);
	if (DelText(Y, C, CP.Col - C) == 0)
	    return 0;
...
int EBuffer::KillWordOrCap()
{
	...
	if (Class == 1) {
		...
		while ((P < L->Count)
		       && (WGETBIT(Flags.CapitalChars, L->Chars[P]) == 1))
		    P++;
	    while ((P < L->Count)
		   && (WGETBIT(Flags.WordChars, L->Chars[P]) == 1)
		   && (WGETBIT(Flags.CapitalChars, L->Chars[P]) == 0))
		P++;
	}
	else
...
int EBuffer::KillWordOrCapPrev()
{
	...
	if (Class == 1) {
		...
		while ((P > 0)
		       && (WGETBIT(Flags.WordChars, L->Chars[P - 1]) == 1)
		       && (WGETBIT(Flags.CapitalChars, L->Chars[P - 1]) == 0))
		    P--;
	    while ((P > 0)
		   && (WGETBIT(Flags.CapitalChars, L->Chars[P - 1]) == 1))
...
int EBuffer::KillToLineEnd()
{
    ...
    if (DelText(Y, CP.Col, LineLen() - CP.Col))
	return 1;
    return 0;
}
...
int EBuffer::BackSpace()
{
	...
	if (BlockKill() == 0)
	    return 0;
	     ...
	     && CP.Col <= BFI(this, BFI_LeftMargin)
	     && CP.Col <= LineIndented(Y)) {
	if (SetPos(LineLen(Y - 1), CP.Row - 1) == 0)
	    return 0;
    }
    else if (CP.Col == 0) {
	if (CP.Row > 0)
	...
	if (BFI(this, BFI_BackSpUnindents) && (LineIndented(Y) == CP.Col)) {
	    ...
	    C1 = C;
	    while (L > 0
		   && (IsLineBlank(L - 1) || (C1 = LineIndented(L - 1)) >= C))
		L--;
	    if (L == 0)
		C1 = 0;
	    if (C1 == C)
	    ...
	    if (C1 > C)
		C1 = C;
	    if (SetPos(C1, CP.Row) == 0)
		return 0;
	    if (C > LineIndented(Y))
		return 0;
	    if (DelText(Y, C1, C - C1) == 0)
		return 0;
		...
		while (C < L->Count &&
		       (L->Chars[C + 1] == 9 || L->Chars[C + 1] == ' '))
		    C++;
	    }

	    if (DelText(Y, ScreenPos(L, C), 1) == 0)
		return 0;
	    if (BFI(this, BFI_Insert) == 0)
		if (InsText(Y, ScreenPos(L, C), 1, " ") == 0)
		    return 0;
	}
    }
...
int EBuffer::Delete()
{
	...
	if (CP.Col >= LineLen(Y))
	    if (CP.Row < VCount - 1) {
		if (SetPos(BFI(this, BFI_LeftMargin), CP.Row + 1) == 0)
		    return 0;
	    }
    }
    if (BFI(this, BFI_Trim))
...
int EBuffer::LineAdd()
{
    if (InsLine(VToR(CP.Row), 1) && MoveDown())
	return 1;
    return 0;
}
...
{
    if (JoinLine(VToR(CP.Row), CP.Col))
	return 1;
    return 0;
}
...
int EBuffer::TypeChar(char aCh)
{					// does abbrev expansion if appropriate
	...
	if (P >= 0 && P <= L->Count) {
		...
		if (ab) {
			...
			if (((EGUI *) gui)->
			    ExecMacro(View->MView->Win, ab->Cmd) == 0)
			    return 0;
		    }
		}
	    }
...
int EBuffer::InsertString(const char *aStr, int aCount)
{
	...
	if (C > BFI(this, BFI_RightMargin)) {
	    ...
	    if (SetPos(CP.Col - C - 1 + BFI(this, BFI_LeftMargin), CP.Row + 1)
		== 0)
		return 0;
	}
    }
...
int EBuffer::InsertSpacesToTab(int TSize)
{
    ...
    if (BFI(this, BFI_Insert) == 0) {
	if (CP.Col < LineLen())
	    if (DelText(VToR(CP.Row), CP.Col, P1 - P) == 0)
		return 0;
    }
    if (InsText(VToR(CP.Row), CP.Col, P1 - P, 0) == 0)
	return 0;
    if (SetPos(P1, CP.Row) == 0)
	return 0;
    return 1;
}
...
int EBuffer::DoWrap(int WrapAll)
{
	...
	if (VToR(CP.Row) != L || L != FirstParaLine) {
		...
		if (CP.Col <= LineIndented(L))
		    if (SetPos(Left, CP.Row) == 0)
			WFAIL(1);
	    Ind = IndentLine(L, Left);
	    if (VToR(CP.Row) == L)
		if (SetPos((CP.Col + Ind > 0) ? CP.Col + Ind : 0, CP.Row) ==
		    0)
		    WFAIL(2);
	    NoChange = 0;
	}
	...
	if (Len < Right) {
		...
		if (!IsLineBlank(L + 2)) {
		    if (JoinLine(L + 1, Left) == 0)
			WFAIL(5);
		}
	    }
	    if (L == VToR(CP.Row) && CP.Col > C) {
		if (SetPos(Left + CP.Col - C - 1, CP.Row + 1) == 0)
		    WFAIL(6);
	    }
	    NoChange = 0;
	    L++;
...
int EBuffer::LineDuplicate()
{
    ...
    if (InsLine(Y, 1) == 0)
	return 0;
    if (InsChars(Y + 1, 0, RLine(Y)->Count, RLine(Y)->Chars) == 0)
	return 0;
    return 1;
}
