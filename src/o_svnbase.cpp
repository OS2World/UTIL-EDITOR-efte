...
{
    if (SvnIgnoreRegexpCount >= MAXREGEXP)
	return 0;
    if ((SvnIgnoreRegexp[SvnIgnoreRegexpCount] = RxCompile(regexp)) == NULL)
	return 0;
...
ESvnBase::ESvnBase(int createFlags, EModel ** ARoot,
		   const char *title):EList(createFlags, ARoot, title)
{
    LineCount = 0;
    Lines = 0;
    Running = 0;
...
void ESvnBase::NotifyPipe(int APipeId)
{
	...
	while (GetLine((char *)line, sizeof(line))) {
		...
		if (RxExec(SvnIgnoreRegexp[i], line, len, line, &RM) == 1)
		    break;
	    if (i == SvnIgnoreRegexpCount)
		ParseLine(line, len);
	}
	if (!Running) {
	    char s[30];
...
void ESvnBase::ShowLine(EView * V, int line)
{
	...
	if (Lines[line]->Buf != 0) {
		...
		if (Lines[line]->Line != -1)
		    ((EBuffer *) ActiveModel)->CenterNearPosR(0,
							      Lines[line]->
							      Line);
	    }
	}
