...
int EEditPort::SetTop(int Col, int Line)
{
    ...
    if (Line >= Buffer->VCount)
	Line = Buffer->VCount - 1;
    if (Line < 0)
	Line = 0;
...
void EEditPort::HandleMouse(TEvent & Event)
{
	...
	case evMouseMove:
		...
		if (Event.Mouse.Buttons == 1) {
			...
			if (Event.Mouse.KeyMask == kfCtrl)
			    Buffer->BlockMarkColumn();
			else if (Event.Mouse.KeyMask == kfAlt)
			    Buffer->BlockMarkLine();
			else
			    Buffer->BlockMarkStream();
			Buffer->BlockUnmark();
...
int EBuffer::ExecCommand(int Command, ExState & State)
{
	...
	&& (Command != ExMoveUp && Command != ExMoveDown)) {
...
int EBuffer::GetString(ExState & State)
{
    ...
    if (View->MView->Win->
	GetStr(Prompt, sizeof(GetStrVars[No]), GetStrVars[No],
	       HIST_POSITION) == 0)
	return 0;
...
int EBuffer::ExpandTemplate(ExState & State)
{
	...
	if (View->MView->Win->
	    GetStr("Filename", MAXPATH, FileName, HIST_PATH) == 0)
	    return 0;
    }
	...
	strcpy(values[num], "");
	if (View->MView->Win->
	    GetStr(prompts[num], sizeof(values[num]), values[num],
		   HIST_DEFAULT) == 0) {
	    RxFree(re);
	    return 0;
	}
...
int EBuffer::MoveToLine(ExState & State)
{
	...
	if (View->MView->Win->
	    GetStr("Goto Line", sizeof(Num), Num, HIST_POSITION) == 0)
	    return 0;
	No = atol(Num);
    }
...
int EBuffer::MoveToColumn(ExState & State)
{
	...
	if (View->MView->Win->GetStr("Goto Column", 8, Num, HIST_POSITION) ==
	    0)
	    return 0;
	No = atol(Num);
    }
...
int EBuffer::FoldCreateByRegexp(ExState & State)
{
	...
	if (View->MView->Win->
	    GetStr("Create Fold Regexp", sizeof(strbuf), strbuf,
		   HIST_REGEXP) == 0)
	    return 0;
    }
...
int EBuffer::PlaceUserBookmark(const char *n, EPoint P)
{
	...
	if (BFI(this, BFI_SaveBookmarks) == 1
	    || BFI(this, BFI_SaveBookmarks) == 2) {
	    if (!Modify())
		return result;		// Never try to save to read-only
	    if (BFI(this, BFI_Undo)) {
		if (PushULong(prev.Row) == 0)
		    return 0;
		if (PushULong(prev.Col) == 0)
		    return 0;
		if (PushUData((void *)n, strlen(n) + 1) == 0)
		    return 0;
		if (PushULong(strlen(n) + 1) == 0)
		    return 0;
		if (PushUChar(ucPlaceUserBookmark) == 0)
		    return 0;
	    }
	}
    }
...
int EBuffer::RemoveUserBookmark(const char *n)
{
	...
	if (BFI(this, BFI_SaveBookmarks) == 1
	    ...
	    if (!Modify())
		return result;		// Never try to save to read-only
	    if (PushULong(p.Row) == 0)
		return 0;
	    if (PushULong(p.Col) == 0)
		return 0;
	    if (PushUData((void *)n, strlen(n) + 1) == 0)
		return 0;
	    if (PushULong(strlen(n) + 1) == 0)
		return 0;
	    if (PushUChar(ucRemoveUserBookmark) == 0)
		return 0;
	}
    }
    return result;
...
int EBuffer::GetUserBookmarkForLine(int searchFrom, int searchForLine,
				    char *&Name, EPoint & P)
{
    int i;
...
int EBuffer::PlaceBookmark(ExState & State)
{
	...
	if (View->MView->Win->
	    GetStr("Place Bookmark", sizeof(name), name, HIST_BOOKMARK) == 0)
	    return 0;
...
int EBuffer::RemoveBookmark(ExState & State)
{
    ...
    if (State.GetStrParam(View, name, sizeof(name)) == 0)
	if (View->MView->Win->
	    GetStr("Remove Bookmark", sizeof(name), name, HIST_BOOKMARK) == 0)
	    return 0;
...
int EBuffer::GotoBookmark(ExState & State)
{
    ...
    if (State.GetStrParam(View, name, sizeof(name)) == 0)
	if (View->MView->Win->
	    GetStr("Goto Bookmark", sizeof(name), name, HIST_BOOKMARK) == 0)
	    return 0;
...
int EBuffer::PlaceGlobalBookmark(ExState & State)
{
    ...
    if (State.GetStrParam(View, name, sizeof(name)) == 0)
	if (View->MView->Win->
	    GetStr("Place Global Bookmark", sizeof(name), name,
		   HIST_BOOKMARK) == 0)
	    return 0;
...
int EBuffer::InsertString(ExState & State)
{
    ...
    if (State.GetStrParam(View, strbuf, sizeof(strbuf)) == 0) {
	if (View->MView->Win->
	    GetStr("Insert String", sizeof(strbuf), strbuf,
		   HIST_DEFAULT) == 0)
	    return 0;
    }
...
int EBuffer::FileSaveAs(ExState & State)
{
	...
	if (View->MView->Win->
	    GetFile("Save As", sizeof(FName), FName, HIST_PATH,
		    GF_SAVEAS) == 0)
	    return 0;
...
int EBuffer::FileWriteTo(ExState & State)
{
    ...
    if (State.GetStrParam(View, FName, sizeof(FName)) == 0)
	if (View->MView->Win->
	    GetFile("Write To", sizeof(FName), FName, HIST_PATH,
		    GF_SAVEAS) == 0)
	    return 0;
...
int EBuffer::BlockReadX(ExState & State, int blockMode)
{
    ...
    if (State.GetStrParam(View, FName, sizeof(FName)) == 0)
	if (View->MView->Win->
	    GetFile("Read block", sizeof(FName), FName, HIST_PATH,
		    GF_OPEN) == 0)
	    return 0;
...
int EBuffer::BlockWrite(ExState & State)
{
    ...
    if (State.GetStrParam(View, FName, sizeof(FName)) == 0)
	if (View->MView->Win->
	    GetFile("Write block", sizeof(FName), FName, HIST_PATH,
		    GF_SAVEAS) == 0)
	    return 0;
...
int EBuffer::Find(ExState & State)
{
	...
	if (View->MView->Win->
	    GetStr
	    ("Options (All/Block/Cur/Delln/Glob/Igncase/Joinln/Rev/Word/regX)",
...
int EBuffer::FindReplace(ExState & State)
{
	...
	if (State.GetStrParam(View, options, sizeof(options)) == 0)
	    if (View->MView->Win->
		GetStr
		("Options (All/Block/Cur/Delln/Glob/Igncase/Joinln/Rev/Splitln/Noask/Word/regX)",
		 sizeof(options), options, HIST_SEARCHOPT) == 0)
		return 0;
...
int EBuffer::ChangeMode(ExState & State)
{
	...
	if (View->MView->Win->
	    GetStr("Mode", sizeof(Mode), Mode, HIST_SETUP) == 0)
	    return 0;
...
int EBuffer::ChangeTabSize(ExState & State)
{
	...
	if (View->MView->Win->
	    GetStr("TabSize", sizeof(Num), Num, HIST_SETUP) == 0)
	    return 0;
	No = atol(Num);
    }
    if (No < 1)
...
int EBuffer::ChangeRightMargin(ExState & State)
{
	...
	if (View->MView->Win->
	    GetStr("RightMargin", sizeof(Num), Num, HIST_SETUP) == 0)
	    return 0;
	No = atol(Num) - 1;
    }
    if (No <= 1)
...
int EBuffer::ChangeLeftMargin(ExState & State)
{
	...
	if (View->MView->Win->
	    GetStr("LeftMargin", sizeof(Num), Num, HIST_SETUP) == 0)
	    return 0;
	No = atol(Num) - 1;
    }
    if (No < 0)
...
int EBuffer::FindTag(ExState & State)
{
	...
	if (View->MView->Win->
	    GetStr("Find tag", sizeof(Tag), Tag, HIST_SEARCH) == 0)
	    return 0;
...
int EBuffer::GetStrVar(int var, char *str, int buflen)
{
    ...
    case mvWord:{
		...
		while ((P > 0) && (C == ChClassK(L->Chars[P - 1])))
		    P--;

		wordBegin = P;

		// search end of word
		while ((P < L->Count) && (C == ChClassK(L->Chars[P])))
		    P++;
