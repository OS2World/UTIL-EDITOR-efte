...
static void SendSelection(XEvent * notify, Atom property, Atom type,
			  unsigned char *data, int len, Bool privateData);
...
static int SetupXWindow(int argc, char **argv)
{
    ...
    if ((display =
	 XtOpenDisplay(app_context, NULL, argv[0], "efte", NULL, 0, &argc,
		       argv)) == NULL) {
	...
	DieError(1, "XeFTE Fatal: could not open display: %s!",
    ...
    win = XCreateWindow(display, DefaultRootWindow(display), initX, initY,
    ...
    sizeHints.flags =
	PResizeInc | PMinSize | PMaxSize | PBaseSize | PWinGravity;
	...
	(display, win, const_cast < char **>(efte16x16_xpm), &icon_pixmap,
	 &icon_shape, NULL) == XpmSuccess)
    {
    ...
    static const char **xpmData[ICON_COUNT] =
	{ (const char **)efte16x16_xpm, (const char **)efte32x32_xpm,
(const char **)efte48x48_xpm, (const char **)efte64x64_xpm };
	...
	if (XpmCreateXpmImageFromData
	    (const_cast < char **>(xpmData[i]), &xpm, NULL) != XpmSuccess)
	        break;
	...
	for (j = 0; j < xpm.ncolors; j++) {
	    ...
	    if (c == NULL)
		c = xpm.colorTable[j].g_color;
	    else if (c == NULL)
		c = xpm.colorTable[j].g4_color;
	    else if (c == NULL)
		c = xpm.colorTable[j].m_color;
	    else if (c == NULL)
		c = xpm.colorTable[j].symbolic;
	    if (c == NULL) {
		// Unknown color
		colors[j] = 0;
	    }
    ...
    if (atom != None) {
	XChangeProperty(display, win, atom, XA_ATOM, 32, PropModeReplace,
			(unsigned char *)winAtoms, 2);
    }
    ...
    if (atom != None) {
	XChangeProperty(display, win, atom, XA_WINDOW, 32, PropModeReplace,
			(unsigned char *)&win, 1L);
    }

    atom = XInternAtom(display, "WM_WINDOW_ROLE", False);
    if (atom != None) {
	XChangeProperty(display, win, atom, XA_STRING, 8, PropModeReplace,
    ...
    if (atom != None) {
	XChangeProperty(display, win, atom, XA_STRING, 8, PropModeReplace,
			(unsigned char *)locale_name, strlen(locale_name));
    }
...
void DrawCursor(int Show)
{
	...
	if (CursorBlink)
	    Show &=
		(CursorLastTime % (CursorFlashInterval * 2)) >
		CursorFlashInterval;
	int attr = p[1];
...
int ConScroll(int Way, int X, int Y, int W, int H, TAttr Fill, int Count)
{
	...
	XCopyArea(display, win, win, GCs[0],
		  X * FontCX,
		  (Y + Count) * FontCY,
		  W * FontCX, (H - Count) * FontCY, X * FontCX, Y * FontCY);
	for (l = 0; l < H - Count; l++)
	...
	XCopyArea(display, win, win, GCs[0],
		  ...
		  Y * FontCY,
		  W * FontCX,
		  (H - Count) * FontCY, X * FontCX, (Y + Count) * FontCY);
	for (l = H - 1; l >= Count; l--)
	    memcpy(CursorXYPos(X, Y + l), CursorXYPos(X, Y + l - Count),
		   2 * W);
...
void ConvertKeyToEvent(KeySym key, KeySym key1, char * /*keyname */ ,
		       char * /*keyname1 */ , int etype, int state,
		       TEvent * Event)
{
    ...
    if (key < 256
	|| (key1 < 256
	    && (myState == kfAlt || myState == (kfAlt | kfShift)))) {
	if (myState & kfAlt)
	    key = key1;
	if (myState == kfShift)
	    myState = 0;
	...
	for (unsigned i = 0; i < (sizeof(key_table) / sizeof(key_table[0]));
	     i++) {
	    long k;
...
void ConvertClickToEvent(int type, int xx, int yy, int button, int state,
...
void ProcessXEvents(TEvent * Event)
{
	...
	if (event.type == PropertyNotify
	    ...
	    for (isi = incrementalSelections; isi;
		 prev_isi = isi, isi = isi->next) {
		if (isi->requestor == event.xproperty.window
		    && isi->property == event.xproperty.atom) {
		    // Found selection request - send more data
		    int send = isi->len - isi->pos;

		    send =
			send <
			SELECTION_XFER_LIMIT ? send : SELECTION_XFER_LIMIT;
		    XChangeProperty(display, isi->requestor, isi->property,
				    isi->type, 8, PropModeAppend,
				    isi->data + isi->pos, send);
		    isi->pos += send;
	...
	UpdateWindow(exposeEvent->x,
		     exposeEvent->y, exposeEvent->width, exposeEvent->height);
	break;
    case GraphicsExpose:
	...
	ConvertClickToEvent(event.type, buttonEvent->x, buttonEvent->y,
			    buttonEvent->button, buttonEvent->state, Event,
			    motionEvent->time);
	break;
    case FocusIn:
	if (i18n_ctx)
	...
	if (!i18n_ctx || event.type == KeyRelease)
	    ...
	    i18n_lookup_sym(keyEvent, keyName, sizeof(keyName), &key,
			    i18n_ctx->xic);
	    if (!key)
		break;
	}
	...
	ConvertKeyToEvent(key, key1, keyName, keyName1, event.type, state,
			  Event);
	break;
    case MotionNotify:
	now = event.xmotion.time;
	ConvertClickToEvent(event.type, motionEvent->x, motionEvent->y, 0,
			    motionEvent->state, Event, motionEvent->time);
	break;
    case ClientMessage:
    ...
    case SelectionRequest:{
		...
		if (event.xselectionrequest.target == proptype_targets) {
		    ...
		    XChangeProperty(display,
				    ...
				    sizeof(type_list) / sizeof(*type_list));
			...
			event.xselectionrequest.property;
			 ...
			 || event.xselectionrequest.target == proptype_text) {
#endif
		    // No conversion, just the string we have (in fact we should convert to ISO Latin-1)
		    SendSelection(&notify, event.xselectionrequest.property,
				  XA_STRING,
				  (CurSelectionData[clip] ?
				   CurSelectionData[clip] : empty),
				  CurSelectionLen[clip], False);
		    notifySent = True;
#ifdef USE_XMB
		}
		else {
		    ...
		    char *text_list[1] =
			{ (char *)(CurSelectionData[clip] ?
				   CurSelectionData[clip] : empty) };

		    XICCEncodingStyle style =
			event.xselectionrequest.target ==
			XA_STRING ? XStringStyle : event.xselectionrequest.
			target ==
			proptype_text ? XStdICCTextStyle : event.
			xselectionrequest.target ==
			proptype_compound_text ? XCompoundTextStyle :
#ifdef X_HAVE_UTF8_STRING
			event.xselectionrequest.target ==
			proptype_utf8_string ? XUTF8StringStyle :
#endif
			(XICCEncodingStyle) - 1;
	    ...
	    if (!notifySent)
		XSendEvent(display, notify.xselection.requestor, False, 0L,
			   &notify);

	    // Now clean too old incremental selections
	    IncrementalSelectionInfo *isi = incrementalSelections, *prev_isi =
		NULL;
	...
	if ((WaitTime == -1 || WaitTime > MouseAutoDelay)
		...
		*Event = LastMouseEvent;
		return 0;
		 ...
		 && (LastMouseEvent.What == evMouseDown
	    ...
	    rc = select(sizeof(fd_set) * 8,
			FD_SET_CAST() & read_fds, NULL, NULL, &timeout);
	    if (rc == 0) {
		LastMouseEvent.What = evMouseAuto;
		*Event = LastMouseEvent;
		return 0;
	    ...
	    rc = select(sizeof(fd_set) * sizeof(char),
			FD_SET_CAST() & read_fds, NULL, NULL, NULL);
	}
	else {
	    timeout.tv_sec = 0;
			  ...
			  unsigned char *data, int len, Bool privateData)
{
	...
	if (isi->data != NULL) {
	    ...
	    XSelectInput(display, notify->xselection.requestor,
			 PropertyChangeMask);

	    // Send total size
	    XChangeProperty(display, notify->xselection.requestor, property,
			    proptype_incr, 32, PropModeReplace,
			    (unsigned char *)&len, 1);
...
static int ConvertSelection(Atom selection, Atom type, int *len, char **data)
{
    ...
    if (!WaitForXEvent(SelectionNotify, &event)
	|| event.xselection.property != prop_selection)
	return -1;
    ...
    retval =
	XGetWindowProperty(display, win, prop_selection, 0, 0, False,
			   AnyPropertyType, &actual_type, &actual_format,
			   &nitems, &bytes_after, &d);
    XFree(d);
    if (retval != Success)
	return -1;
	...
	XGetWindowProperty(display, win, prop_selection, 0, 8, True,
			   proptype_incr, &actual_type, &actual_format,
			   &nitems, &bytes_after, &d);
	buffer_len = *(int *)d;
	...
	while (1) {
	    ...
	    XGetWindowProperty(display, win, prop_selection, 0,
			       nitems + bytes_after, True, type, &actual_type,
			       &actual_format, &nitems, &bytes_after, &d);
	...
	*data = NULL;			// NULL indicates failure
	retval =
	    XmbTextPropertyToTextList(display, &text, &list, &list_count);
	XFree(d);
	if (retval >= 0) {
...
#ifdef USE_XMB
#ifdef X_HAVE_UTF8_STRING
	    if (ConvertSelection(clip, proptype_utf8_string, len, data) == 0)
		return 0;
#endif
...
GUI::GUI(int &argc, char **argv, int XSize, int YSize)
{
	...
	if (strcmp(argv[c], "-display") == 0) {
		...
		XParseGeometry(argv[++c], &initX, &initY, &ScreenCols,
			       &ScreenRows);
	...
	else if (strcmp(argv[c], "-wclass") == 0) {
	    if (c + 1 < argc) {
		snprintf(res_class, sizeof(res_class), "%s", argv[++c]);
	    }
	}
	else if (strcmp(argv[c], "-wname") == 0) {
	    if (c + 1 < argc) {
...
char ConGetDrawChar(int idx)
{
    ...
    if (!tab) {
	tab =
	    GetGUICharacters("X11",
			     "\x0D\x0C\x0E\x0B\x12\x19____+>\x1F\x01\x12 ");
    }
