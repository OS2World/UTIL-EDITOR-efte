...
int EBuffer::LoadFrom(const char *AFileName)
{
    ...
    if ((BFI(this, BFI_SaveFolds) != 0) || BFI(this, BFI_SaveBookmarks) == 1
	...
	if (BFS(this, BFS_CommentStart) == 0)
	    ...
	    len_start = strlen(BFS(this, BFS_CommentStart));
	if (BFS(this, BFS_CommentEnd) == 0)
	    len_end = 0;
	else
	    len_end = strlen(BFS(this, BFS_CommentEnd));
	...
	for (l = RCount - 1; l >= 0; l--) {
		...
		for (int where = 1; where < 3; where++) {
		    ...
		    open = -1;
		    level = -1;
		    if (BFI(this, BFI_SaveFolds) != where
			&& BFI(this, BFI_SaveBookmarks) != where)
			continue;
		    if (where == 1) {
			pos = 0;
		    }
			...
			if (len_end != 0
			    && memcmp(LL[l]->Chars + pos,
				      BFS(this, BFS_CommentEnd),
				      len_end) != 0)
			    continue;
			// Locate position of comment start - skip bookmarks and fold
			if (BFI(this, BFI_SaveBookmarks) == 2 && pos - 10 >= 0
			    && LL[l]->Chars[pos - 1] == 'b') {
			    char numbuf[5];
			    int i;
			...
			|| (len_start != 0
			    && memcmp(LL[l]->Chars + pos,
				      BFS(this, BFS_CommentStart),
				      len_start) != 0))
			continue;
		    // Check comment length - leave empty in file
		    if (LL[l]->Count - pos == len_start + len_end)
			continue;
		    startpos = pos;
		    pos += len_start;
		    ...
		    // Now get bookmarks
		    if (BFI(this, BFI_SaveBookmarks) == where
			&& (pos + len_end + 10 <= LL[l]->Count)
			&& memcmp(LL[l]->Chars + pos, "BOOK", 4) == 0) {
			int error = 0;
			int i, col, startBook;
			char numbuf[5];
			char buf[256];
			...
			while (pos + len_end + 6 + 6 <= LL[l]->Count) {
				...
				buf[i] = 0;
				pos += i;
				if (PlaceUserBookmark(buf, EPoint(l, col)) ==
				    0)
				    goto fail;
			    }
			    if (LL[l]->Chars[pos] == 'x') {
		    ...
		    if (where == 2 && LL[l]->Count != pos + len_end)
			continue;
		    pos += len_end;
...
int EBuffer::SaveTo(char *AFileName)
{
	...
	if (FileStatus.st_size != StatBuf.st_size ||
	    ...
	    switch (View->MView->Win->
		    Choice(GPC_ERROR, "File Changed on Disk", 2, "&Save",
			   "&Cancel", "%s", FileName)) {
	    case 0:
		break;
	    case 1:
    ...
    if (BFS(this, BFS_CommentStart))
	len_start = strlen(BFS(this, BFS_CommentStart));
    if (BFS(this, BFS_CommentEnd))
	len_end = strlen(BFS(this, BFS_CommentEnd));
	...
	if (BFI(this, BFI_SaveBookmarks) == 1
	    ...
	    while ((bindex =
		    GetUserBookmarkForLine(bindex, l, bname, bpos)) != -1) {
		// Skip too long bookmarks
		if (strlen(bname) > 256
		    || blen + strlen(bname) + 6 + 6 > sizeof(book))
		    continue;
		blen +=
		    sprintf(book + blen, "%04x%02x%s", bpos.Col,
			    (unsigned int)strlen(bname), bname);
	    }
	    if (blen != 4) {
	...
	for (int what = 1; what < 3; what++) {
		...
		if (len_start) {
		    if (fwrite(BFS(this, BFS_CommentStart), 1, len_start, fp)
			!= len_start)
			goto fail;
		    ByteCount += len_start;
		}
		if (BFI(this, BFI_SaveFolds) == what && foldlen) {
		    if (fwrite(fold, 1, foldlen, fp) != foldlen)
			goto fail;
		    ByteCount += foldlen;
		}
		if (BFI(this, BFI_SaveBookmarks) == what && blen) {
		    if (fwrite(book, 1, blen, fp) != blen)
			goto fail;
		    ByteCount += blen;
		}
		if (len_end) {
		    if (fwrite(BFS(this, BFS_CommentEnd), 1, len_end, fp) !=
			len_end)
			goto fail;
		    ByteCount += len_end;
		}
  ...
  fail:
	...
	View->MView->Win->Choice(GPC_ERROR, "Error", 1, "O&K",
				 "Error renaming backup file to original!");
    }
    else {
	View->MView->Win->Choice(GPC_ERROR, "Error", 1, "O&K",
				 "Error writing file, backup restored.");
    }
    return 0;
  erroropen:
    View->MView->Win->Choice(GPC_ERROR, "Error", 1, "O&K",
			     "Error writing %s (errno=%d).", AFileName,
			     errno);
    return 0;
}
