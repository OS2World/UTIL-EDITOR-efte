...
static int DefineColor(char *name, char *value)
{
	...
	return 0;
    colors =
	(struct _color *)realloc(colors,
				 sizeof(struct _color) * (colorCount + 1));
    assert(colors != 0);
...
static char *GetColor(CurPos & cp, char *name)
{
	...
	*c++ = 0;
    ...
    if (!isxdigit(name[0]) &&
	name[1] != ' ' && !isxdigit(name[2]) && name[3] != 0) {
...
int NewCommand(CurPos & cp, const char *Name)
{
	...
	Name = "";
    CFteMacros =
	(DefinedMacro *) realloc(CFteMacros,
				 sizeof(DefinedMacro) * (1 + CCFteMacros));
...
static int ParseCommands(CurPos & cp, char *Name)
{
	...
	Fail(cp, "%s has already been defined in %s:%i", Name,
	     CFteMacros[Cmd ^ CMD_EXT].FileName,
	...
	if (p == P_DOT) {
	    ...
	    GetOp(cp, P_VARIABLE);
	    if (Parse(cp) != P_WORD)
		Fail(cp, "Syntax error (variable name expected)");
	    Word w;

	    if (GetWord(cp, w) != 0)
		Fail(cp, "Syntax error (bad variable name)");
	    long var = Lookup(CfgVar, w);

	    if (var == -1)
		Fail(cp, "Unrecognized variable: %s", w);
...
static int ParseConfigFile(CurPos & cp)
{
	...
	case P_WORD:
	    ...
	    case K_COLORIZE:{
			...
			case K_KEYWORD:{
				...
				while (1) {
				    ...
				    if (strlen(kname) >= CK_MAXLEN)
					Fail(cp, "Keyword name is too long");
			...
			case K_HSTATE:{
				...
				if (Parse(cp) != P_NUMBER)
				    Fail(cp, "state index expected");
				stateno = GetNumber(cp);
				if (stateno != LastState + 1)
				    Fail(cp, "invalid state index");
			...
			case K_HTRANS:{
				...
				if (Parse(cp) != P_NUMBER)
				    Fail(cp, "next_state index expected");
				next_state = GetNumber(cp);
				if (Parse(cp) != P_COMMA)
				    Fail(cp, "',' expected");
				GetOp(cp, P_COMMA);
				if (Parse(cp) != P_STRING)
				    Fail(cp, "match options expected");
				opts = GetString(cp);
				if (Parse(cp) != P_COMMA)
				    Fail(cp, "',' expected");
				GetOp(cp, P_COMMA);
				if (Parse(cp) != P_STRING)
				    Fail(cp, "match string expected");
				match = GetString(cp);
				PutNumber(cp, CF_HTRANS, next_state);
				match_opts = 0;
				if (strchr(opts, '^'))
				    match_opts |= MATCH_MUST_BOL;
				if (strchr(opts, '$'))
				    match_opts |= MATCH_MUST_EOL;
				//if (strchr(opts, 'b')) match_opts |= MATCH_MUST_BOLW;
				//if (strchr(opts, 'e')) match_opts |= MATCH_MUST_EOLW;
				if (strchr(opts, 'i'))
				    match_opts |= MATCH_NO_CASE;
				if (strchr(opts, 's'))
				    match_opts |= MATCH_SET;
				if (strchr(opts, 'S'))
				    match_opts |= MATCH_NOTSET;
				if (strchr(opts, '-'))
				    match_opts |= MATCH_NOGRAB;
				if (strchr(opts, '<'))
				    match_opts |= MATCH_TAGASNEXT;
				if (strchr(opts, '>'))
				    match_opts &= ~MATCH_TAGASNEXT;
				//if (strchr(opts, '!')) match_opts |= MATCH_NEGATE;
				if (strchr(opts, 'q'))
				    match_opts |= MATCH_QUOTECH;
				if (strchr(opts, 'Q'))
				    match_opts |= MATCH_QUOTEEOL;
				if (strchr(opts, 'x'))
				    match_opts |= MATCH_REGEXP;
			...
			case K_HWTYPE:
				...
				opts = GetString(cp);
				if (strchr(opts, 'i'))
				    options |= STATE_NOCASE;
				if (strchr(opts, '<'))
				    options |= STATE_TAGASNEXT;
				if (strchr(opts, '>'))
				    options &= ~STATE_TAGASNEXT;
				if (strchr(opts, '-'))
				    options |= STATE_NOGRAB;
	    ...
	    case K_MODE:{		// mode::
			...
			Fail(cp, "Syntax error");
		    if (GetWord(cp, ObjName) != 0)
			Fail(cp, "Parse failed, expecting a word");
			...
			if (Parse(cp) != P_WORD)
			    Fail(cp, "Syntax error");
			if (GetWord(cp, UpMode) != 0)
			    Fail(cp, "Parse failed, expecting a mode name");
		    }
		    PutString(cp, CF_PARENT, UpMode);
		    if (Parse(cp) != P_OPENBRACE)
			Fail(cp, "'{' expected");
			...
			if (GetWord(cp, w) != 0)
			    Fail(cp,
				 "Parse failed, expecting a variable name");
			if (Parse(cp) != P_ASSIGN)
			    Fail(cp, "'=' expected");
			GetOp(cp, P_ASSIGN);
...
#       define SEARCH_PATH_LEN 6
	...
	snprintf(dirs[2], MAXPATH, "%s/share/efte/local/%s", EFTE_INSTALL_DIR,
		 CfgName);
	snprintf(dirs[3], MAXPATH, "/etc/efte/local/%s", CfgName);
	snprintf(dirs[4], MAXPATH, "%s/share/efte/config/%s",
